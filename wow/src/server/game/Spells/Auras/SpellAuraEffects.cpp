...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_POTION)
    {
        ...
        {
            if (caster && caster->HasAura(53042) && caster->HasSpell(GetSpellInfo()->Effects[0].TriggerSpell))
            {
                amount += amount * 0.3f;
            }
        }
...
{
    ASSERT(caster);
    float crit_chance = GetBase()->GetCritModifier();
    ...
    {
        if ((*itr)->IsAffectedOnSpell(m_spellInfo))
            if (crit_chance && roll_chance_f(crit_chance))
                return true;
    }

    // Rupture - since 3.3.3 can crit
    if (m_spellInfo->SpellIconID == 500 && m_spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE)
        return (crit_chance && roll_chance_f(crit_chance));
...
void AuraEffect::HandleModTaunt(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (CharmInfo* charminfo = target->GetCharmInfo())
        charminfo->HandleTaunt(apply, true);
    ...
    if (CharmInfo* charminfo = target->GetCharmInfo())
        charminfo->HandleTaunt(apply, false);
}
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    else
    {
        ...
        switch (auraId)
        {
            ...
            case 69783: // Rotface Ooze Flood
            case 69797: // Rotface Ooze Flood
            case 69799: // Rotface Ooze Flood
            case 69802: // Rotface Ooze Flood
            {
                if (target->GetPositionZ() > 365.0f) // @todo: is there a hook that could do this?
                    return;
                break;
            }
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        damage = caster->SpellDamageBonusDone(target, GetSpellInfo(), damage, DOT, GetBase()->GetStackAmount(), GetBase(), false);
    ...
    caster->CalcAbsorbResist(target, GetSpellInfo()->GetSchoolMask(), DOT, damage, &absorb, &resist, GetSpellInfo(), -1);
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    caster->CalcAbsorbResist(target, GetSpellInfo()->GetSchoolMask(), DOT, damage, &absorb, &resist, m_spellInfo, -1);
