...
Aura* Aura::Create(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, Item* castItem, uint64 casterGUID)
{
    ...
    //if (!caster && casterGUID)
        //return NULL; // caster doesn't exist
...
m_isRemoved(false), m_isSingleTarget(false), m_isUsingCharges(false)
{
    ...
    m_staticDmgPctMod = 1.0f;
    m_staticCritMod = 0.0f;
        ...
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
                {
                    ...
                    {
                        int32 damage = caster->SpellDamageBonusDone(target, GetSpellInfo(), GetEffect(0)->GetAmount(), DOT);
                        int32 basepoints0 = float(aurEff->GetAmount() / 100.0f) * (GetEffect(0)->GetTotalTicks() * damage);
                        caster->CastCustomSpell(target, 63675, &basepoints0, NULL, NULL, true, NULL, GetEffect(0));

                        basepoints0 = target->SpellDamageBonusTaken(caster, GetSpellInfo(), basepoints0, SPELL_DIRECT_DAMAGE);
                    ...
                    (GetEffect(EFFECT_0)->GetAmount() <= 0))
                {
                    ...
                    if (AuraEffect* dummy = caster->GetDummyAuraEffect(SPELLFAMILY_MAGE, 2945, 0))
                        if (roll_chance_i(dummy->GetSpellInfo()->ProcChance))
                            target->CastSpell(target, 55080, true, NULL, GetEffect(0));
                }
                break;
