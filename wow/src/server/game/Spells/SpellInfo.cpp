...
float SpellEffectInfo::CalcRadius(Unit* caster, Spell* spell) const
{
    ...
    if (caster)
    {
        if (caster->GetTypeId() == TYPEID_UNIT && (caster->GetEntry() == 40001 || caster->GetEntry() == 40135) && caster->HasAura(70507))
            radius = 7.0f + (float)caster->GetAuraCount(70507) * 0.5f; // hack, i think blizzard used RadiusPerLevel to do this, but I can't seem to get it working
        else
        {
...
bool SpellInfo::HasTriggerSpell() const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(Effects[i].TriggerSpell))
            return true;

    return false;
}

bool SpellInfo::HasPositiveTriggerSpell() const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(Effects[i].TriggerSpell))
...
bool SpellInfo::CanDispelAura(SpellInfo const* aura) const
{
    ...
    // if both auras have SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY then we should return false
    if ((Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) && !(aura->Attributes & SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY))
        return true;
...
bool SpellInfo::IsDstSpell() const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (Effects[i].TargetA.GetTarget() == TARGET_UNIT_DEST_AREA_ENTRY || Effects[i].TargetA.GetTarget() == TARGET_UNIT_DEST_AREA_ENEMY
         || Effects[i].TargetA.GetTarget() == TARGET_DEST_CASTER          || Effects[i].TargetA.GetTarget() == TARGET_DEST_DB
         || Effects[i].TargetA.GetTarget() == TARGET_DEST_HOME            || Effects[i].TargetA.GetTarget() == TARGET_DEST_DYNOBJ_ENEMY
         || Effects[i].TargetA.GetTarget() == TARGET_DEST_DYNOBJ_ALLY     || Effects[i].TargetA.GetTarget() == TARGET_UNIT_DEST_AREA_ALLY
         || Effects[i].TargetA.GetTarget() == TARGET_UNIT_DEST_AREA_PARTY || Effects[i].TargetA.GetTarget() == TARGET_DEST_CASTER_SUMMON
         || Effects[i].TargetA.GetTarget() == TARGET_DEST_CASTER_UNK_36   || Effects[i].TargetA.GetTarget() == TARGET_DEST_TARGET_ENEMY
         || Effects[i].TargetA.GetTarget() == TARGET_DEST_DEST_RADIUS     || Effects[i].TargetA.GetTarget() == TARGET_DEST_CHANNEL_CASTER
         || Effects[i].TargetA.GetTarget() == TARGET_UNK_DEST_AREA_UNK_107)
         return true;

        if (Effects[i].TargetB.GetTarget() == TARGET_UNIT_DEST_AREA_ENTRY || Effects[i].TargetB.GetTarget() == TARGET_UNIT_DEST_AREA_ENEMY
         || Effects[i].TargetB.GetTarget() == TARGET_DEST_CASTER          || Effects[i].TargetB.GetTarget() == TARGET_DEST_DB
         || Effects[i].TargetB.GetTarget() == TARGET_DEST_HOME            || Effects[i].TargetB.GetTarget() == TARGET_DEST_DYNOBJ_ENEMY
         || Effects[i].TargetB.GetTarget() == TARGET_DEST_DYNOBJ_ALLY     || Effects[i].TargetB.GetTarget() == TARGET_UNIT_DEST_AREA_ALLY
         || Effects[i].TargetB.GetTarget() == TARGET_UNIT_DEST_AREA_PARTY || Effects[i].TargetB.GetTarget() == TARGET_DEST_CASTER_SUMMON
         || Effects[i].TargetB.GetTarget() == TARGET_DEST_CASTER_UNK_36   || Effects[i].TargetB.GetTarget() == TARGET_DEST_TARGET_ENEMY
         || Effects[i].TargetB.GetTarget() == TARGET_DEST_DEST_RADIUS     || Effects[i].TargetB.GetTarget() == TARGET_DEST_CHANNEL_CASTER
         || Effects[i].TargetB.GetTarget() == TARGET_UNK_DEST_AREA_UNK_107)
         return true;
    }
    return false;
}
...
SpellCastResult SpellInfo::CheckShapeshift(uint32 form) const
{
    ...
    if (form > 0)
    {
        ...
        if (!shapeInfo)
        {
            TC_LOG_ERROR("spells", "GetErrorAtShapeshiftedCast: unknown shapeshift %u from spell %u", form, Id);
            return SPELL_CAST_OK;
        }
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit, bool ignoreAurReq) const
{
    ...
    if (!ignoreAurReq && !caster->IsVehicle() && !(caster->GetCharmerOrOwner() == target))
    {
    ...
    if (!ignoreAurReq && TargetAuraSpell && !unitTarget->HasAura(sSpellMgr->GetSpellIdForDifficulty(TargetAuraSpell, caster)))
        return SPELL_FAILED_TARGET_AURASTATE;

    if (!ignoreAurReq && ExcludeTargetAuraSpell && unitTarget->HasAura(sSpellMgr->GetSpellIdForDifficulty(ExcludeTargetAuraSpell, caster)))
        return SPELL_FAILED_TARGET_AURASTATE;
...
bool SpellInfo::CheckTargetCreatureType(Unit const* target) const
{
    ...
    if (target->GetUInt32Value(UNIT_CREATED_BY_SPELL) == 8177) //bypass creature type check for grounding totem
      return true;
        ...
        case SPELLFAMILY_GENERIC:
        {
            ...
            else
            {
                ...
                switch (firstRankSpellInfo->Id)
                {
                    ...
                    case 14204: // Enrage
                    case 57522: // Enrage
                    case 57516: // Enrage (Imp. defensive stance)
...
bool SpellInfo::HasDirectDamage() const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (Effects[i].Effect == SPELL_EFFECT_SCHOOL_DAMAGE ||
            Effects[i].Effect == SPELL_EFFECT_INSTAKILL ||
            Effects[i].Effect == SPELL_EFFECT_ENVIRONMENTAL_DAMAGE)
            return true;
...
bool SpellInfo::IsSpellDelaySpell() const
{
    switch (Id)
    {
        ...
        case 33786: // Cyclone
        // PRIEST
        case 64044: // Psychic Horror - Trigger and Stun
    ...
    {
        case SPELLFAMILY_PALADIN:
            if (SpellFamilyFlags[0] == 0x800) // Hammer of Justice
                return true;
            break;
        case SPELLFAMILY_ROGUE:
            //           Kidney Shot                       Sap
            if (SpellFamilyFlags[0] == 0x200000 || SpellFamilyFlags[0] == 0x80)
                return true;
            break;
        case SPELLFAMILY_DRUID:
            //            Bash                            Pounce                          Maim                      Faerie Fire            Hibernate
            if (SpellFamilyFlags[0] == 0x2000 || SpellFamilyFlags[0] == 0x20000 || SpellFamilyFlags[1] == 0x80 || SpellIconID == 109 || SpellIconID == 44)
                return true;
            break;
        case SPELLFAMILY_PRIEST:
            //         Psychic Scream                Shadow Word Death
            if (SpellFamilyFlags[0] == 0x10000 || SpellFamilyFlags[1] == 0x2)
                return true;
            break;
        case SPELLFAMILY_MAGE:
            //         Polymorph                       Deep Freeze                Dragon's Breath                  Fire Blast
            if (SpellFamilyFlags[0] == 0x1000000 || SpellIconID == 2939 || SpellFamilyFlags[0] == 0x800000 || SpellFamilyFlags[0] == 0x2)
                return true;
            break;
        case SPELLFAMILY_WARLOCK:
            //            Fear                        Howl of Terror                  Shadowfury
            if (SpellFamilyFlags[1] == 0x400 || SpellFamilyFlags[1] == 0x8 || SpellFamilyFlags[1] == 0x1000)
                return true;
            break;
        default:
            break;
...
bool SpellInfo::IsSilenceDelaySpell() const
{
    switch (Id)
    {
        ...
        case 50613: // Arcane Torrent - Runic Power
        // WARRIOR
    ...
    {
        case SPELLFAMILY_DEATHKNIGHT:
            if (SpellFamilyFlags[0] == 0x200) // Strangulate
                return true;
            break;
        case SPELLFAMILY_MAGE:
            if (SpellFamilyFlags[1] == 0x40000000) // Silenced - Improved Counterspell
                return true;
            break;
        default:
            break;
...
bool SpellInfo::IsMiscDelaySpell() const
{
    ...
    {
        // GENERIC
        case 56488: // Global Thermal Sapper Charge
        // PRIEST
