...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(SpellInfo const* spellProto, SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, uint32 procFlags, uint32 procExtra, bool active) const
{
    ...
    */
    ...
    // HoT's should be included in this procFlag aswell
    if (procFlags & PROC_FLAG_DONE_PERIODIC)
    {
        if (procSpell->GetCategory() == 0 && procSpell->Dispel == 1
            && (procSpell->Effects[EFFECT_0].ApplyAuraName == SPELL_AURA_PERIODIC_HEAL || procSpell->Effects[EFFECT_1].ApplyAuraName == SPELL_AURA_PERIODIC_HEAL || procSpell->Effects[EFFECT_2].ApplyAuraName == SPELL_AURA_PERIODIC_HEAL))
            return true;
    }
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
            ...
            case 44401: // Missile Barrage
                ...
                break;
            case 64085: // Vampiric Touch (Dispel Damage)
                spellInfo->SpellFamilyFlags[0] |= 0x2000000;
                break;
            ...
            case 77846: // Twilight Cutter
                spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_100_YARDS); // 100yd
                break;
            case 75884: // Combustion(HC)
            case 75883: // Combustion(N)
            case 75876: // Consumption(HC)
            case 75875: // Consumption(N)
                spellInfo->RangeEntry =  sSpellRangeStore.LookupEntry(13);
                ...
                spellInfo->SpellLevel = 255; // cant miss
                break;
            case 75509: // Twilight Mending
                spellInfo->AttributesEx6 |= SPELL_ATTR6_CAN_TARGET_INVISIBLE;
                spellInfo->AttributesEx2 |= SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS;
                break;
            case 74795: // Mark of Consumption
            case 74567: // Mark of Combustion
                spellInfo->Dispel = DISPEL_NONE; // cant be dispelled
                spellInfo->SpellLevel = 255;
                break;
            case 74792: // Soul Consumption
            case 74562: // Fiery Combustion
                spellInfo->Dispel = DISPEL_ALL;
                spellInfo->SpellLevel = 255;
                break;
                /*
            ...
            case 60256: // Collect Sample
                ...
                break;
            case 53489: // The Art of War
            case 59578: // The Art of War Rank 2
            ...
            case 62012: // Turkey Caller
				spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_100_YARDS);    // 0yd
                break;
            default:
                break;
        }
        ...
        switch (spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_PALADIN:
                ...
                // Exorcism and Flash of Light should not reset melee swing timer.
                else if ((spellInfo->SpellFamilyFlags[0] & 0x40000000 || spellInfo->GetCategory() == 19) && spellInfo->Attributes != 0x1D0)
                    spellInfo->AttributesEx2 &= SPELL_ATTR2_NOT_RESET_AUTO_ACTIONS;
                break;
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                // Pestilence
                if (spellInfo->SpellFamilyFlags[1]&0x10000)
                    spellInfo->AttributesEx6 |= SPELL_ATTR6_CAN_TARGET_INVISIBLE;
                break;
        }
    }
