...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (center != referer && !(m_spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS))
    {
        for (std::list<WorldObject*>::iterator itr = targets.begin(); itr != targets.end();)
        {
            if (!(*itr)->IsWithinLOS(center->GetPositionX(), center->GetPositionY(), center->GetPositionZ()) && (*itr)->GetTypeId() != TYPEID_GAMEOBJECT && (*itr)->GetTypeId() != TYPEID_DYNAMICOBJECT
                && !((*itr)->GetTypeId() == TYPEID_UNIT && (*itr)->GetEntry() == 36980 && m_caster->ToPlayer())) // ice tomb hack
                targets.erase(itr++);
            else
                ++itr;
        }
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (!CheckDelayInterrupts(unit, target))
        return;
    ...
    if (missInfo != SPELL_MISS_NONE && missInfo != SPELL_MISS_REFLECT && missInfo != SPELL_MISS_MISS)
    {
        if (target->timeDelay)
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                m_originalCaster->SpellDamageBonusDone(unit, m_spellInfo, 0, DOT, m_spellAura->GetStackAmount(), m_spellAura, true);
    ...
    if (m_spellAura && m_spellAura->IsBreakableByAnyDamage(m_spellAura->GetCaster()))
    {
...
bool Aura::IsBreakableByAnyDamage(Unit* exclude) const
{
    uint32 excludeAura = 0;
    if (Spell* currentChanneledSpell = exclude ? exclude->GetCurrentSpell(CURRENT_CHANNELED_SPELL) : NULL)
        excludeAura = currentChanneledSpell->GetSpellInfo()->Id; //Avoid self interrupt of channeled Crowd Control spells like Seduction

    if (GetId() != excludeAura)
    {
        if (const SpellInfo* spellInfo = GetSpellInfo())
        {
            if (   spellInfo->HasAura(SPELL_AURA_MOD_CONFUSE)
            ...
            {
                if (spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_TAKE_DAMAGE)
                    return true;
            }
        }
    }
...
void Spell::cast(bool skipCheck)
{
    ...
    if ((m_spellInfo->Speed > 0.0f && !m_spellInfo->IsChanneled()) || m_spellInfo->AttributesEx4 & SPELL_ATTR4_UNK4 || m_spellInfo->Id == 14157 || // 14157: Ruthlessness (Neeeded?)
        ...
        for (std::vector<int32>::const_iterator i = spell_triggered->begin(); i != spell_triggered->end(); ++i)
            ...
            else
            {
                ...
                else
                {
                    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                    {
                        SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(*i);
                        if (triggeredSpellInfo)
                        {
                            if (ihit->missCondition == SPELL_MISS_NONE || (ihit->missCondition == SPELL_MISS_IMMUNE && !(m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : m_caster)->IsImmunedToSpell(triggeredSpellInfo)))
                                m_caster->CastSpell(m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : m_caster, *i, true);
                            else if (ihit->missCondition == SPELL_MISS_REFLECT && m_targets.GetUnitTarget())
                                m_targets.GetUnitTarget()->CastSpell(m_caster, *i, true);
                        }
                    }
                }
    ...
    for (std::list<TargetInfo>::iterator ihit= m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        if (ihit->processed == false)
        {
            if (single_missile || ihit->timeDelay <= t_offset)
            {
                ...
                if (unit)
                {
                    if (ihit->missCondition != SPELL_MISS_MISS && ihit->missCondition != SPELL_MISS_REFLECT)
                    {
                        SpellMissInfo missTemp = m_caster->SpellHitResult(unit, m_spellInfo, m_canReflect);
                        if (missTemp != SPELL_MISS_MISS && missTemp != SPELL_MISS_REFLECT)
                            ihit->missCondition = missTemp;
                    }
...
SpellCastResult Spell::CheckCast(bool strict)
{
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (GetSpellInfo()->NeedsExplicitUnitTarget())
                {
                    ...
                    m_preGeneratedPath.SetPathLengthLimit(m_spellInfo->GetMaxRange(true, m_caster, this) + m_caster->GetObjectSize() + target->GetObjectSize());
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (m_spellInfo->Effects[i].TargetA.GetTarget() == TARGET_UNIT_CASTER || m_spellInfo->Effects[i].TargetB.GetTarget() == TARGET_UNIT_CASTER)
            if (GetCaster()->IsImmunedToSpell(m_spellInfo))
                return SPELL_FAILED_NOT_READY;
    // all ok
    return SPELL_CAST_OK;
}
...
SpellCastResult Spell::CheckCasterAuras() const
{
	...
	if (m_spellInfo->Id == 33206 && !m_caster->HasAura(63248) || m_caster->HasAura(60210))
		usableInStun = true;

	if (m_caster->HasAura(60210))
		return SPELL_CAST_OK;
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
    ...
    switch (m_spellInfo->Effects[eff].Effect)
    {
        ...
        default:                                            // normal case
            ...
            switch (m_spellInfo->Effects[eff].TargetB.GetTarget())
            {
                case TARGET_UNIT_SRC_AREA_ALLY:
                case TARGET_UNIT_SRC_AREA_PARTY:
                case TARGET_UNIT_CASTER_AREA_RAID:
            ...
            // no TargetA and no TargetB matching conditions, check LoS
            if (target != m_caster && !target->IsWithinLOSInMap(caster) && !target->HasAuraType(SPELL_AURA_SPELL_MAGNET))
                return false;
            break;
    }
...
bool Spell::CheckDelayInterrupts(Unit* unit, TargetInfo* target) const
{
    if (getState() != SPELL_STATE_DELAYED || m_spellInfo->IsPositive())
        return true;

    if ((getMSTime() - target->timeDelay) <= (unit->m_lastSanctuaryTime + 50))
    {
    ...
    if (m_spellInfo->Mechanic == MECHANIC_STUN && (getMSTime() - target->timeDelay) <= unit->m_lastBlinkTime)
    {
