...
void Spell::EffectEnvironmentalDMG(SpellEffIndex /*effIndex*/)
{
    ...
    m_caster->CalcAbsorbResist(unitTarget, m_spellInfo->GetSchoolMask(), SPELL_DIRECT_DAMAGE, damage, &absorb, &resist, m_spellInfo, -1);
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x200 && m_spellInfo->GetCategory() == 1209)
                {
                    ...
                    float block_value_multiplier = 1.0f;
                    ...
                    for (Unit::AuraApplicationMap::iterator i = auras.begin(); i != auras.end();++i)
                    {
                        ...
                        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                        {
                            ...
                            if ((aurEff && aurEff->GetSpellModifier() && m_spellInfo->IsAffectedBySpellMod(aurEff->GetSpellModifier())) || (aurEff && !aurEff->GetSpellModifier()))
                            {
                                switch (aurEff->GetAuraType())
                                {
                                    case SPELL_AURA_MOD_SHIELD_BLOCKVALUE_PCT:
                                        block_value_multiplier += float((aurEff->GetAmount()) / 100.0f);
                                        break;
                                }
                            }
                        }
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1]&0x00040000)
                {
                    ...
                    float min, max;
                    m_caster->CalculateMinMaxDamage(BASE_ATTACK, false, false, min, max);
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (triggered_spell_id == m_spellInfo->Id)
    {
        TC_LOG_ERROR("spells", "Spell::EffectTriggerSpell spell %u tried to trigger itself", m_spellInfo->Id);
        return;
    }
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    switch(m_spellInfo->Id)
    {
        ...
        case 66549:
        {
            ...
            m_caster->GetCreatureListWithEntryInGrid(TriggerList, 22515, 60.0f);

            float dist = 0.0f;
            Creature* target = NULL;
            for (std::list<Creature*>::iterator itr = TriggerList.begin(); itr != TriggerList.end(); ++itr)
            {
                float _dist = m_caster->GetDistance2d((*itr));
                if (_dist > dist)
                {
                    dist = _dist;
                    target = (*itr);
...
void Spell::CalculateJumpSpeeds(uint8 i, float speedRate, float &speedXY, float &speedZ)
{
    ...
    {
        uint32 dist = speedRate;
        speedRate = m_caster->GetSpeedRate(MOVE_RUN);
    ...
    else
    {
        if (m_spellInfo->Effects[i].MiscValue)
            ...
            speedZ = 10.0f;
        speedXY = speedRate * 10.0f / speedZ;
    }
}
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    // some fishing pole bonuses except Glow Worm which lasts full hour
        ...
        duration = 86400;                                     // 10 mins
    // shaman rockbiter enchantments
    else if (m_spellInfo->SpellVisual[0] == 0)
        duration = 86400;                                    // 30 mins
    else if (m_spellInfo->Id == 29702)
        duration = 86400;                                     // 5 mins
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    if (OldSummon)
    {
        if (petentry == 0 || OldSummon->GetEntry() == petentry)
        {
            ...
            OldSummon->StopMoving();
            OldSummon->AttackStop();
            OldSummon->InterruptNonMeleeSpells(false);

            if (CharmInfo* charmInfo = OldSummon->GetCharmInfo())
            {
            ...
            OldSummon->GetMotionMaster()->MoveFollow(owner, PET_FOLLOW_DIST, OldSummon->GetFollowAngle());
            OldSummon->RemoveAllPositiveAuras();
            OldSummon->RemoveAllNegativeAuras();
            OldSummon->ResetAllPowers();
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if (casterAreaEntry && !(casterAreaEntry->flags & AREA_FLAG_ALLOW_DUELS) && caster->GetMapId() != 603 && caster->GetMapId() != 615)
    {
    ...
    if (targetAreaEntry && !(targetAreaEntry->flags & AREA_FLAG_ALLOW_DUELS) && target->GetMapId() != 603 && caster->GetMapId() != 615)
    {
        SendCastResult(SPELL_FAILED_NO_DUELING);            // Dueling isn't allowed here
        return;
    }
...
void Spell::EffectTitanGrip(SpellEffIndex /*effIndex*/)
{
    ...
    {
        plrCaster->SetCanTitanGrip(true);
        if (plrCaster->HasTwoHandWeaponInOneHand() && !plrCaster->HasAura(49152))
            plrCaster->CastSpell(plrCaster, 49152, true);
    }
}
