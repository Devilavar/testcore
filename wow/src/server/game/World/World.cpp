...
World::World()
{
    ...
    auth_cnt = 0;// part of auth attack prevention in WorldSocket
...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_int_configs[CONFIG_CHAT_CHANNEL_LEVEL_REQ] = 0;
    m_int_configs[CONFIG_CHAT_WHISPER_LEVEL_REQ] = 0;
    m_int_configs[CONFIG_CHAT_SAY_LEVEL_REQ] = 0;
	m_int_configs[CONFIG_TRADE_LEVEL_REQ] = 0;
    m_int_configs[CONFIG_TICKET_LEVEL_REQ] = 0;
    m_int_configs[CONFIG_AUCTION_LEVEL_REQ] = 0;
    m_int_configs[CONFIG_MAIL_LEVEL_REQ] = 0;
    m_int_configs[CONFIG_CHAT_CHANNEL_PLAYEDTIME_REQ] = 0;
    m_int_configs[CONFIG_CHAT_WHISPER_PLAYEDTIME_REQ] = 0;
    m_int_configs[CONFIG_CHAT_SAY_PLAYEDTIME_REQ] = 0;
    m_int_configs[CONFIG_TRADE_PLAYEDTIME_REQ] = 0;
    m_int_configs[CONFIG_TICKET_PLAYEDTIME_REQ] = 0;
    m_int_configs[CONFIG_AUCTION_PLAYEDTIME_REQ] = 0;
    m_int_configs[CONFIG_MAIL_PLAYEDTIME_REQ] = 0;
    ...
    m_bool_configs[CONFIG_MMAP_LOAD_METHOD] = false;
    ...
    m_bool_configs[CONFIG_ARENA_2v2_TEAM_ENABLE]                     = true;
    m_bool_configs[CONFIG_ARENA_3v3_TEAM_ENABLE]                     = true;
    m_bool_configs[CONFIG_ARENA_5v5_TEAM_ENABLE]                     = true;
    m_int_configs[CONFIG_ARENA_SPECTATOR_MIN_RATING]                 = 1000;
    ...
    m_int_configs[CONFIG_CHAT_FILTER_ENABLE]        = 0;
    m_int_configs[CONFIG_CHAT_FILTER_MAX_MSGS]      = 1;
    m_bool_configs[CONFIG_DISPLAY_TOTAL_WHO]        = true;
	m_int_configs[CONFIG_EXTERNAL_MAIL] = 0;
	m_int_configs[CONFIG_EXTERNAL_MAIL_INTERVAL] = 0;

    m_int_configs[CONFIG_SPELL_DELAY_SPELL_TIME]    = 100;
    m_int_configs[CONFIG_MOVEMENT_DELAY_SPELL_TIME] = 10;
    m_int_configs[CONFIG_SILENCE_DELAY_SPELL_TIME]  = 10;
    m_int_configs[CONFIG_MISC_DELAY_SPELL_TIME]     = 100;
    ...
    m_bool_configs[CONFIG_XMOG_ON_BG_END] = false;
    ...
    m_int_configs[CONFIG_MOVEMENT_CHECKS_ACCESSLEVEL] = sConfigMgr->GetIntDefault("Anticheat.skipgmlvl", SEC_CONSOLE);
    m_int_configs[CONFIG_MOVEMENT_CHECKS_MASK] = sConfigMgr->GetIntDefault("MovementChecker.EnableMask", 0);
    m_bool_configs[CONFIG_ANTICHEAT_ENABLE] = false; // PASSIVE ANTICHEAT ENABLED?? NO.
    ...
    m_int_configs[CONFIG_WINTERGRASP_CUSTOMREWARD_ITEM] = 0;
    m_int_configs[CONFIG_WINTERGRASP_CUSTOMREWARD_AMOUNT] = 0;

    m_bool_configs[CONFIG_INIT_SKILLS] = false;
    m_float_configs[CONFIG_FAC_CHANGE_DIFF] = 0.0f;

    // Auth Flooding settings - 0 = Disable, 1+ = Enabled (seconds allowed between auth packets)
    auth_flood_mode = sConfigMgr->GetIntDefault("Auth_Flood_Mode", 0);
...
void World::SetInitialWorldSettings()
{
    ...
    QueryResult result = LoginDatabase.PQuery("SELECT gamebuild FROM realmlist WHERE id = '%d'", realmID);
    ASSERT(result && "Realm doesn't exist in realmlist table!");

    Field* fields = result->Fetch();
    m_realmbuild = fields[0].GetUInt32();
    ...
    TC_LOG_INFO("server.loading", "Loading InitSpells...");
    ...
    TC_LOG_INFO("server.loading", "Deleting Old Characters...");
    Player::DeleteOldCharacters();

    // Delete all custom channels which haven't been used for PreserveCustomChannelDuration days.
    TC_LOG_INFO("server.loading", "Deleting Old Chat Channels...");
    ...
    bool preloadMMaps = sWorld->getBoolConfig(CONFIG_MMAP_LOAD_METHOD);

    if (preloadMMaps)
    {
        for (uint32 map = 0; map <= 724; ++map)
        {
            if (!sMapStore.LookupEntry(map))
                continue;

            TC_LOG_INFO("server.loading", "Loading MMap data for map %u...", map);
            for (uint32 x = 0; x < 64; ++x)
                for (uint32 y = 0; y < 64; ++y)
                    MMAP::MMapFactory::createOrGetMMapManager()->loadMap((sWorld->GetDataPath() + "mmaps").c_str(), map, x, y);
        }
    }
...
void World::RecordTimeDiff(const char *text, ...)
{
    ...
    if (diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        va_end(ap);
        TC_LOG_FATAL("misc", "Difftime %s: %u.", str, diff);
    }
...
void World::Update(uint32 diff)
{
    ...
    if (m_int_configs[CONFIG_INTERVAL_LOG_UPDATE] && diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        {
            TC_LOG_FATAL("misc", "Update time diff: %u. Players online: %u.", m_updateTimeSum / m_updateTimeCount, GetActiveSessionCount());
...
void World::KickAllLess(uint16 sec)
{
    // session not removed at kick and will removed in next update tick
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
...
void World::LoadDBAllowedSecurityLevel()
{
    ...
    if (result)
        SetPlayerSecurityLimit(result->Fetch()->GetUInt8());
}

void World::SetPlayerSecurityLimit(uint16 _sec)
{
