...
{
    if (sWorld->getBoolConfig(CONFIG_MMAP_LOAD_METHOD))
        return;
...
{
    if (!Trinity::IsValidMapCoord(x) || !Trinity::IsValidMapCoord(y))
    {
        TC_LOG_ERROR("maps", "Map::GetGrid: Passed invalid map coordinates (X: %f, Y: %f)", x, y);
        return NULL;
    }
...
bool Map::IsInAir(uint32 phasemask, float x, float y, float z, bool vmap/*=true*/, float maxSearchDist/*=DEFAULT_HEIGHT_SEARCH*/) const
{
    float groundz = GetWaterOrGroundLevel(x, y, z);
    float floorz = GetHeight(phasemask, x, y, z, vmap, maxSearchDist);
    float maplevel = groundz > floorz ? groundz : floorz;

    return (z - maplevel) > 3.0f;
}
...
void BattlegroundMap::RemovePlayerFromMap(Player* player, bool remove)
{
    ...
    SharedVisionList viscopy(player->GetSharedVisionList());
    for (SharedVisionList::const_iterator i = viscopy.begin(); i != viscopy.end(); ++i)
        (*i)->SetViewpoint(player, false);
