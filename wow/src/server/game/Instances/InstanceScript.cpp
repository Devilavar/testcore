...
bool InstanceScript::IsEncounterInProgress() const
{
    for (std::vector<BossInfo>::const_iterator itr = bosses.begin(); itr != bosses.end(); ++itr)
        if (itr->state == IN_PROGRESS)
            return false; // vremenen hack za da mogat da vlizat v insta kogato se biqt (temp fixva combat buga v raidovete)
...
// [HACK] Complete Achievement for all players in instance
void InstanceScript::CompleteAchievement(uint32 achievement)
{
    ...
    if (!AE)
    {
       TC_LOG_ERROR("misc", "Error in Code: The *HACK* Function [CompleteAchievement] returned not existing achievement %u , devs check this achievment id in the scripts...", achievement);
        return;
    }
