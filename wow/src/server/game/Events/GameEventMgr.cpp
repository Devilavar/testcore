...
void GameEventMgr::LoadFromDB()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Game Event Vendor Additions Data...");
    {
        ...
        //                                               0           1     2     3         4         5
        QueryResult result = WorldDatabase.Query("SELECT eventEntry, guid, item, maxcount, incrtime, ExtendedCost, sort FROM game_event_npc_vendor ORDER BY guid, slot ASC");
        ...
        else
        {
            ...
            do
            {
                ...
                newEntry.sort = fields[6].GetUInt32();
                ...
                if (!sObjectMgr->IsVendorItemValid(newEntry.entry, newEntry.item, newEntry.maxcount, newEntry.incrtime, newEntry.ExtendedCost, newEntry.sort, NULL, NULL, event_npc_flag))
                    continue;
...
void GameEventMgr::UpdateEventNPCVendor(uint16 event_id, bool activate)
{
    ...
    {
        if (activate)
            sObjectMgr->AddVendorItem(itr->entry, itr->item, itr->maxcount, itr->incrtime, itr->ExtendedCost, itr->sort, false);
        else
            sObjectMgr->RemoveVendorItem(itr->entry, itr->item, itr->sort, false);
    }
}
