...
void PointMovementGenerator<T>::DoInitialize(T* unit)
{
    ...
    if (unit->ToCreature() && (unit->IsInAir(i_x, i_y, i_z) || unit->IsInAir()))
...
void PointMovementGenerator<T>::DoFinalize(T* unit)
{
    ...
    if (unit->ToCreature() && !unit->IsInAir())
        unit->RemoveUnitMovementFlag(MOVEMENTFLAG_FLYING);
...
void EffectMovementGenerator::Initialize(Unit* unit)
{
    if (unit->movespline->isParabolic())
        unit->AddUnitState(UNIT_STATE_JUMPING|UNIT_STATE_JUMPING_MOVE);
}

void EffectMovementGenerator::Finalize(Unit* unit)
{
    if (unit->movespline->isParabolic())
        unit->ClearUnitState(UNIT_STATE_JUMPING|UNIT_STATE_JUMPING_MOVE);

    if (unit->ToCreature() && unit->ToCreature()->AI())
