...
void MotionMaster::DirectClean(bool reset, uint8 slot)
{
    if (slot != MAX_MOTION_SLOT && _top != slot)
    {
        MovementGenerator* movegen = Impl[slot];
        pop(slot);
        if (movegen)
            DirectDelete(movegen);
        return;
    }
...
void MotionMaster::MoveKnockbackFrom(float srcX, float srcY, float speedXY, float speedZ)
{
    ...
    for (uint8 i = 0; i < 5; ++i)
    {
        switch (i)
        {
            case 0: _owner->GetNearPoint(_owner, x, y, z, _owner->GetObjectSize(), dist, _owner->GetAngle(srcX, srcY) + M_PI);        break;
            case 1: _owner->GetNearPoint(_owner, x, y, z, _owner->GetObjectSize(), dist * 0.8, _owner->GetAngle(srcX, srcY) + M_PI);  break;
            case 2: _owner->GetNearPoint(_owner, x, y, z, _owner->GetObjectSize(), dist * 0.5, _owner->GetAngle(srcX, srcY) + M_PI);  break;
            case 3: _owner->GetNearPoint(_owner, x, y, z, _owner->GetObjectSize(), dist * 0.25, _owner->GetAngle(srcX, srcY) + M_PI); break;
            case 4: _owner->GetNearPoint(_owner, x, y, z, _owner->GetObjectSize(), dist * 0.1, _owner->GetAngle(srcX, srcY) + M_PI);  break;
        }
...
void MotionMaster::MoveJump(float x, float y, float z, float speedXY, float speedZ, uint32 id)
{
    ...
    if (CharmInfo* charminfo = _owner->GetCharmInfo())
