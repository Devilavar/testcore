...
#define MAX_PATH_DISTANCE       0
#define MAX_POINT_PATH_POINTS   74
#define MAX_WHITE_PATH_LENGTH   300
...
class PathGenerator
{
    public:
        ...
        void SetPathLengthLimit(float distance) { _pathLengthLimit = distance; }
    ...
    private:
        ...
        dtPolyRef _pathPolyRefs[MAX_POINT_PATH_POINTS];   // array of detour polygon references
        ...
        float _pathLengthLimit; // limit point path length
        ...
        void NormalizePath();
        void NormalizePath(float* path, uint32 pathSize);
        ...
        dtStatus FindSmartPath(const float* startPos, const float* endPos,
                             const dtPolyRef* polyPath, uint32 polyPathSize,
                             float* smartPath, int* pointCount, float maxPathLength);

        bool GenerateWhitePoints(float* pathPoints, int* pathPointsSize, float* returnPath);
        bool GenerateSmartPoints(float* pathPoints, int* pathPointsSize, float* smartPath);
        float CalculateLength(float* pathPoints, uint32 pathPointsSize);
};
