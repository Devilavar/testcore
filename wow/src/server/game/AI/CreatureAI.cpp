...
{
    if (!me->IsInCombat() && (!me->HasReactState(REACT_AGGRESSIVE) || me->isWorldBoss()))
        return false;

    if (me->HasReactState(REACT_AGGRESSIVE) || (me->HasReactState(REACT_DEFENSIVE) && me->IsInCombat()))
    {
        if (Unit* victim = me->SelectVictim(me->IsInCombat()))
            AttackStart(victim);
        return me->GetVictim();
    }
    else if (me->getThreatManager().isThreatListEmpty() && me->IsInCombat())
    {
        EnterEvadeMode();
        return false;
    }
