    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_ADD_QUEST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    ...
                    {
                        if ((*itr)->ToPlayer()->AddQuest(q, NULL))
        ...
        case SMART_ACTION_TELEPORT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    if (e.action.teleport.ignoreCombat == 1) // 1 true 0 false
                    {
                        if ((*itr)->ToPlayer()->IsInCombat())
                        {
                            (*itr)->ToPlayer()->GetSession()->SendNotification("You cannot teleport while in combat.");
                            continue;
                        }
                    }
