...
void UnitAI::ClearCharmInfoFlags()
{
...
Unit* UnitAI::SelectTarget(SelectAggroTarget targetType, uint32 position, float dist, bool playerOnly, int32 aura, std::list<Unit*> exclude)
{
    return SelectTarget(targetType, position, DefaultTargetSelector(me, dist, playerOnly, aura, exclude));
}
