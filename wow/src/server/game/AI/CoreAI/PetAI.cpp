...
Unit* PetAI::SelectNextTarget(bool allowAutoSelect) const
{
    ...
    if (!me->getThreatManager().isThreatListEmpty())
        if (Unit* myAttacker = me->getThreatManager().getHostilTarget())
    ...
    if (me->HasReactState(REACT_AGGRESSIVE) && allowAutoSelect && !me->GetCharmInfo()->IsReturning())
        if (Unit* nearTarget = me->SelectNearestTargetInAttackDistance(MAX_AGGRO_RADIUS, true, true))
            return nearTarget;
...
void PetAI::HandleReturnMovement()
{
    ...
    if (me->GetCharmInfo()->HasCommandState(COMMAND_STAY))
    {
        if (!me->GetCharmInfo()->IsAtStay() && !me->GetCharmInfo()->IsReturning())
        {
            ...
            me->GetMotionMaster()->Clear(true, MOTION_SLOT_ACTIVE);
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    if (me->GetCharmInfo()->IsLockedTarget() && target == me->GetVictim())
        return true;
