...
struct DefaultTargetSelector : public std::unary_function<Unit*, bool>
{
    ...
    bool m_playerOnly;
    int32 m_aura;
    std::list<Unit*> m_exclusions;

    // unit: the reference unit
    // dist: if 0: ignored, if > 0: maximum distance to the reference unit, if < 0: minimum distance to the reference unit
    // playerOnly: self explaining
    // aura: if 0: ignored, if > 0: the target shall have the aura, if < 0, the target shall NOT have the aura
    DefaultTargetSelector(Unit const* unit, float dist, bool playerOnly, int32 aura, std::list<Unit*> exclude) : me(unit), m_dist(dist), m_playerOnly(playerOnly), m_aura(aura), m_exclusions(exclude) { }
    ...
    bool operator()(Unit const* target) const
    {
        ...
        for (std::list<Unit*>::const_iterator itr = m_exclusions.begin(); itr != m_exclusions.end(); ++itr)
            if (target->GetGUID() == (*itr)->GetGUID())
                return false;
...
class UnitAI
{
    ...
    public:
        ...
        Unit* SelectTarget(SelectAggroTarget targetType, uint32 position, float dist, bool playerOnly, int32 aura, std::list<Unit*> exclude);
