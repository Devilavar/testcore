...
class ObjectMgr
{
    ...
    public:
        ...
        */
        ...
        bool IsCustomExtendedCost(uint32 ExtendedCost) const { return ExtendedCost > 2997; }
        ...
        void AddVendorItem(uint32 entry, uint32 item, int32 maxcount, uint32 incrtime, uint32 extendedCost, uint32 sort, bool persist = true); // for event
        bool RemoveVendorItem(uint32 entry, uint32 item, uint32 sort, bool persist = true); // for event
        bool IsVendorItemValid(uint32 vendor_entry, uint32 item, int32 maxcount, uint32 ptime, uint32 ExtendedCost, uint32 sort, Player* player = NULL, std::set<uint32>* skip_vendors = NULL, uint32 ORnpcflag = 0) const;
