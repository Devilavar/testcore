...
int ObjectMgr::LoadReferenceVendor(int32 vendor, int32 item, std::set<uint32> *skip_vendors)
{
    ...
    do
    {
        ...
        else
        {
            ...
            uint32 sort         = fields[4].GetUInt32();

            if (!IsVendorItemValid(vendor, item_id, maxcount, incrtime, ExtendedCost, sort, NULL, skip_vendors))
                continue;

            VendorItemData& vList = _cacheVendorItemStore[vendor];

            vList.AddItem(item_id, maxcount, incrtime, ExtendedCost, sort);
            ++count;
        }
    } while (result->NextRow());
...
void ObjectMgr::LoadVendors()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT entry, item, maxcount, incrtime, ExtendedCost, sort FROM npc_vendor ORDER BY entry, slot, sort ASC");
    if (!result)
    {
    ...
    do
    {
        ...
        else
        {
            ...
            uint32 sort         = fields[5].GetUInt32();

            if (!IsVendorItemValid(entry, item_id, maxcount, incrtime, ExtendedCost, sort, NULL, &skip_vendors))
                continue;
...
void ObjectMgr::AddVendorItem(uint32 entry, uint32 item, int32 maxcount, uint32 incrtime, uint32 extendedCost, uint32 sort, bool persist /*= true*/)
{
    VendorItemData& vList = _cacheVendorItemStore[entry];
    vList.AddItem(item, maxcount, incrtime, extendedCost, sort);
    ...
    if (persist)
    {
        ...
        stmt->setUInt32(5, sort);

        WorldDatabase.Execute(stmt);
    }
}

bool ObjectMgr::RemoveVendorItem(uint32 entry, uint32 item, uint32 sort, bool persist /*= true*/)
{
    CacheVendorItemContainer::iterator  iter = _cacheVendorItemStore.find(entry);
    if (iter == _cacheVendorItemStore.end())
        return false;

    if (!iter->second.RemoveItem(item, sort))
        return false;
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 item_id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, uint32 sort, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
    ...
    if (vItems->FindItemCostPair(item_id, ExtendedCost, sort))
    {
        if (player)
...
void ObjectMgr::LoadInitSpells()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Failed to load InitSpells. DB table `init_spells` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded InitSpells in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
