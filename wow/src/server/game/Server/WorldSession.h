...
enum ScheduledEvents
{
    SESSION_EVENT_DISPLAY_ACCOUNT_MSG  = 0,
    SESSION_EVENT_MAX_EVENTS           = 1,
};
...
class WorldSession
{
    public:
        ...
        void Wait(uint32 time) { m_sessionWait += time; }
        uint32 GetWaitTime() { return m_sessionWait; }
        void UpdateWait(uint32 diff) { diff <= m_sessionWait ? m_sessionWait -= diff : m_sessionWait = 0; }

        void ScheduleEvent(uint32 id, uint32 delaytime);
        ...
        void SendListInventory(uint64 guid, uint32 sort = 0);
    ...
    private:
        ...
        // scheduled events
        typedef std::pair<uint32, uint32> _ScheduledEvents;
        typedef std::list<_ScheduledEvents> _ScheduledEventsList;
        _ScheduledEventsList ScheduledEvents;

        typedef std::list<AddonInfo> AddonsList;

        uint32 m_sessionWait;
        ...
        time_t timeLastChInviteCommand;
