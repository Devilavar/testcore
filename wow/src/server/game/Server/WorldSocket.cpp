...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    if (!CanConnectTo(clientBuild, sWorld->GetRealmBuild()))
    {
        packet.Initialize(SMSG_AUTH_RESPONSE, 1);
        packet << uint8(AUTH_REJECT);
        SendPacket(packet);

        TC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: Account: %s [%s] tried to log into a realm that doesn't support their client.", account.c_str(), GetRemoteAddress().c_str());
        return -1;
    }
    ...
    // Prevent attacker from auth spamming to crash server. More than 1 attempt within Auth_Flood_Mode time is blocked.
    int l;
    int cur_idx = 0;
    if (sWorld->auth_flood_mode)// is auth flood protection enabled?
    {
        bool auth_spam_flag = false;// flag auth attacks
        uint32 aid = fields[0].GetUInt32();
        uint32 auth_time = sWorld->GetGameTime();// we'll timestamp each auth attempt

        for (l=0;l <= sWorld->auth_cnt; l++)
        {
            if (sWorld->login_counter[l][0] != aid)
                continue;
            else
            {
                if ((auth_time - sWorld->login_counter[l][1]) < sWorld->auth_flood_mode)
                {
                    TC_LOG_ERROR("network", "WorldSocket::HandleAuthSession: [Account id:%u %s IP:%s] blocked for auth flooding!", aid, account.c_str(), GetRemoteAddress().c_str());
                    CloseSocket();
                    return -1;
                }
                else
                    sWorld->login_counter[l][1] = auth_time;// give them another try
                auth_spam_flag = true;// flag for possible auth flood
                cur_idx = l;// save current array index
            }
        }
        if (!auth_spam_flag)// add auth account for next attempt
        {
            sWorld->login_counter[sWorld->auth_cnt][0] = aid;
            sWorld->login_counter[sWorld->auth_cnt][1] = auth_time;// timestamp 1st auth attempt
            cur_idx = sWorld->auth_cnt;// save current array index
            sWorld->auth_cnt++;
        }
    }
    else// if we disable auth_flood_mode while world is running, remove anyone still in flood array
    {
        for (l=0; l<= sWorld->auth_cnt; l++)
        {
           sWorld->login_counter[l][0] = 0;
        }
        sWorld->auth_cnt = 0;
    }
    ...
    uint16 allowedAccountSec = sWorld->GetPlayerSecurityLimit();
    TC_LOG_DEBUG("network", "Allowed Level: %u Player Level %u", allowedAccountSec, security);
    if (allowedAccountSec > SEC_PLAYER && security < allowedAccountSec)
    {
    ...
    // This wasn't an auth flood so cleanup auth attack array for next candidate
    if (sWorld->auth_flood_mode && sWorld->auth_cnt)
    {
        sWorld->auth_cnt--;
        sWorld->login_counter[cur_idx][0] = 0;
    }
