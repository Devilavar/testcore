...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
    ...
    bool validEmptyAuthedOpcode = true;//validate empty packets/opcodes for status_authed exploits
    uint16 opcode;
    ...
    {
        opcode = packet->GetOpcode();//put in variable to save some cpu time on all these GetOpcode() calls

        if (!AntiDOS.EvaluateOpcode(*packet))
        {
            KickPlayer();
        }
        else if (opcode >= NUM_MSG_TYPES)
        {
            TC_LOG_ERROR("network.opcode", "Received non-existed opcode %s from %s", GetOpcodeNameForLogging(opcode).c_str()
                            , GetPlayerInfo().c_str());
            sScriptMgr->OnUnknownPacketReceive(m_Socket, WorldPacket(*packet));
        }
        else
        {
            OpcodeHandler &opHandle = opcodeTable[opcode];
            try
            {
                switch (opHandle.status)
                {
                    ...
                    case STATUS_TRANSFER:
                        ...
                        break;
                    case STATUS_AUTHED:
                        //empty packets here should only be allowed with certain opcodes
                        if (!packet->size())
                        {
                            switch (opcode)
                            {
                                ...
                                case CMSG_READY_FOR_ACCOUNT_DATA_TIMES:
                                   validEmptyAuthedOpcode=true; //these are valid empty opcodes
                                   break;
                                default:
                                    validEmptyAuthedOpcode=false;
                            }

                            if (!validEmptyAuthedOpcode)
                            {
                                deletePacket=true;
                                // add code here to do action to attacker
                                m_Socket->CloseSocket(); // if we don't close socket, they can continue to spam empty packets in the same session
                                TC_LOG_ERROR("network", "(STATUS_AUTHED) Packet:%u %u %u Opcode:%u %s %s IP:%s", (uint32)packet->size(), (uint32)packet->rpos(), (uint32)packet->wpos(), opcode, opHandle.name, GetPlayerInfo().c_str(), GetRemoteAddress().c_str());
                                break;
                            }
                        }
                        ...
                        break;
                    case STATUS_NEVER:
                        TC_LOG_ERROR("network.opcode", "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(opcode).c_str()
                            , GetPlayerInfo().c_str());
                        break;
                    case STATUS_UNHANDLED:
                        TC_LOG_DEBUG("network.opcode", "Received not handled opcode %s from %s", GetOpcodeNameForLogging(opcode).c_str()
                            , GetPlayerInfo().c_str());
                        break;
                }
            }
            catch (ByteBufferException const&)
            {
                TC_LOG_ERROR("misc", "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i. Skipped packet.",
                        opcode, GetRemoteAddress().c_str(), GetAccountId());
                packet->hexlike();
                m_Socket->CloseSocket();
            }
        }
        ...
        if (processedPackets++ >= limit)
        {
            if (_recvQueue.size() > 1000)
            {
                KickPlayer();
                TC_LOG_ERROR("network", "Packet DoS detected from %s", GetPlayerInfo().c_str());
            }
            break; // process them on next loop
        }
...
void WorldSession::ScheduleEvent(uint32 id, uint32 delaytime)
{
    if (id > SESSION_EVENT_MAX_EVENTS)
    {
        TC_LOG_ERROR("misc", "WorldSession::ScheduleEvent: Tried to schedule an event with an invalid event id: %u", id);
        return;
    }

    ScheduledEvents.push_back(_ScheduledEvents(id, getMSTime() + delaytime));
}

void WorldSession::HandleScheduledEvents()
{
    if (ScheduledEvents.empty())
        return;

    for (std::list<_ScheduledEvents>::iterator itr = ScheduledEvents.begin(); itr != ScheduledEvents.end();)
    {
        if (itr->second <= getMSTime())
        {
            switch (itr->first)
            {
                case SESSION_EVENT_DISPLAY_ACCOUNT_MSG:
                    if (GetPlayer() && GetPlayer()->IsInWorld())
                        GetPlayer()->SendAccountMessages();
                    break;
                default:
                    break;
            }

            ScheduledEvents.erase(itr++);
        }
        else
            ++itr;
    }
...
bool WorldSession::HasPermission(uint32 permission)
{
    switch (permission)
    {
        case rbac::RBAC_PERM_SKIP_CHECK_CHARACTER_CREATION_HEROIC_CHARACTER:
            break;
