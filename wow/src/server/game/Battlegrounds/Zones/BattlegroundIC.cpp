...
void BattlegroundIC::HandleKillPlayer(Player* player, Player* killer)
{
    ...
    factionReinforcements[player->GetTeamId()] -= (GetMaxPlayersPerTeam() / GetPlayersCountByTeam(killer->GetTeam()));
...
WorldSafeLocsEntry const* BattlegroundIC::GetClosestGraveYard(Player* player)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
        if (nodePoint[i].faction == player->GetTeamId())
            if (nodePoint[i].nodeState == (player->GetTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
                nodes.push_back(i);
