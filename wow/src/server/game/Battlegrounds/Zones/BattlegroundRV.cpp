...
void BattlegroundRV::PostUpdateImpl(uint32 diff)
{
    ...
    if (GetBattleTime() >= 20 * IN_MILLISECONDS) // 20 seconds for elevator to reach top
    {
        if (UnderMapCheckDiff <= diff)
        {
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            {
                if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                {
                    if (player->GetPositionZ() < (float)BG_RV_GROUND_LEVEL)
                    {
                        if (player->GetDistance2d(GetBgMap()->GetGameObject(BgObjects[6])) < 2.5f || player->GetDistance2d(GetBgMap()->GetGameObject(BgObjects[7])) < 2.5f)
                            if (GetBgMap()->GetGameObject(BgObjects[6])->GetGoState() == GO_STATE_ACTIVE)
                                player->NearTeleportTo(player->GetPositionX(), player->GetPositionY(), (float)BG_RV_GROUND_LEVEL + 5.0f, player->GetOrientation(), false);
                            else
                                player->NearTeleportTo(player->GetPositionX(), player->GetPositionY(), (float)BG_RV_GROUND_LEVEL, player->GetOrientation(), false);
                        else if (player->GetDistance2d(GetBgMap()->GetGameObject(BgObjects[10])) < 5.0f || player->GetDistance2d(GetBgMap()->GetGameObject(BgObjects[11])) < 5.0f)
                        {
                            if (GetBgMap()->GetGameObject(BgObjects[10])->GetGoState() == GO_STATE_ACTIVE)
                                player->NearTeleportTo(player->GetPositionX(), player->GetPositionY(), (float)BG_RV_GROUND_LEVEL + 8.0f, player->GetOrientation(), false);
                            else
                                player->NearTeleportTo(player->GetPositionX(), player->GetPositionY(), (float)BG_RV_GROUND_LEVEL, player->GetOrientation(), false);
                        }
                        else
...
{
    if (player->GetMapId() != GetMapId()) // handled in PostUpdateImpl, but we still need this for arena spectator
