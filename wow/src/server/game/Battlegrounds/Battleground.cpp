...
void Battleground::Update(uint32 diff)
{
    ...
    switch (GetStatus())
    {
        case STATUS_WAIT_JOIN:
            ...
            TotalTime += diff;
            PrepareTime += diff;
            m_ArmoryStartTime = time_t(time(NULL)); // will actually be one tick off, but meh
            // sLog->outError(LOG_FILTER_SQL, "PrepareTime: %u, TotalTime: %u", PrepareTime, TotalTime);
            break;
        case STATUS_IN_PROGRESS:
            ...
            TotalTime += diff;
            BattleTime += diff;
            m_ArmoryEndTime = time_t(time(NULL)); // will actually be one tick off, but meh
            // sLog->outError(LOG_FILTER_SQL, "BattleTime: %u, TotalTime: %u", BattleTime, TotalTime);
            break;
        case STATUS_WAIT_LEAVE:
            ...
            TotalTime += diff;
            FinishTime += diff;
            // sLog->outError(LOG_FILTER_SQL, "FinishTime: %u, TotalTime: %u", FinishTime, TotalTime);
            break;
        default:
            break;
    }
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    /*
    // disable wintrading on the same pc -- needs regex implentation due to DDoS IP
    if (isArena() && isRated()) // must do this outside because the data set here is called in other places
    {
        std::vector<std::string> ipContainer;
        for(BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
        {
            ...
            std::string playerip = player->GetSession()->GetRemoteAddress().c_str();

            if (!std::regex_match(playerip.c_str(), std::regex("194\\.28\\.157\\.([0-9][0-9][0-9]|[0-9][0-9]|[0-9])"))) // if player ip isn't ddos ip
                ipContainer.push_back(playerip);
        }

        for (uint8 i = 0; i != ipContainer.size(); i++)
        {
            for (uint8 _i = 0; _i != ipContainer.size(); _i++)
            {
                if (ipContainer[i] == ipContainer[_i] && i != _i)
                {
                    isWintrading = true;
                    continue; // no need to keep looping through ips
                }
            }
            if (isWintrading)
                continue; // no need to keep looping through ips
    ...
    */
    ...
    if (isArena() && isRated())
    {
        ...
        if (isWintrading)
            TC_LOG_DEBUG("bg.arena", "Arena match Type: %u for Team1Id: %u - Team2Id: %u ended with no rating change, IP match of players detected.", m_ArenaType, m_ArenaTeamIds[TEAM_ALLIANCE], m_ArenaTeamIds[TEAM_HORDE]);

        if (winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam && BattleTime > 0 && !isWintrading)
        {
            if (winner != WINNER_NONE)
            {
                ...
                for(BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
                {
                    ...
                    sql_query << "INSERT INTO armory_game_chart VALUES ('" << gameID << "', '" << resultTeamID << "', '" << plr->GetGUID() << "', '" << changeType << "', '" << ratingChange  << "', '" << resultRating << "', '" << itr->second->DamageDone << "', '" << itr->second->Deaths << "', '" << itr->second->HealingDone << "', '" << itr->second->DamageTaken << "', '" << itr->second->HealingTaken << "', '" << itr->second->KillingBlows << "', '" << m_MapId << "', '" << m_ArmoryStartTime << "', '" << m_ArmoryEndTime << "')";
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (itr->second.OfflineRemoveTime)
        {
            ...
            if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam && BattleTime > 0 && !isWintrading)
            {
                if (team == winner)
        ...
        if (isArena() && isRated() && winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam && BattleTime > 0 && !isWintrading)
        {
            ...
            {
                case 2:
                    if (frand(1.0f, 100.0f) <= float(team == winner ? 12.5f : 6.25f))
                        player->AddItem(15000, 1);
                    break;
                case 3:
                    if (frand(1.0f, 100.0f) <= float(team == winner ? 25.0f : 12.5f))
                        player->AddItem(15000, 1);
                    break;
                case 5:
                    if (frand(1.0f, 100.0f) <= float(team == winner ? 50.0f : 25.0f))
                        player->AddItem(15000, 1);
                    break;
            }
        }
        else if (isBattleground() && sWorld->getBoolConfig(CONFIG_XMOG_ON_BG_END))
            if (frand(1.0f, 100.0f) <= float(team == winner ? 50.0f : 25.0f))
                player->AddItem(15000, 1);
        ...
        // Reward winner team
        if (team == winner && BattleTime > 0 && !isWintrading)
        {
        ...
        {
            if (IsRandom() || BattlegroundMgr::IsBGWeekend(GetTypeID()) && BattleTime > 0 && !isWintrading)
...
uint32 Battleground::GetFreeSlotsForTeam(uint32 Team, bool basic) const
{
    // if BG is starting ... invite anyone
    if (GetStatus() == STATUS_WAIT_JOIN || basic)
