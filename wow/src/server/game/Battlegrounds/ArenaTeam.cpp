...
uint32 ArenaTeam::GetPoints(uint32 memberRating)
{
    ...
    if (Type == ARENA_TEAM_2v2)
        ...
        points *= 0.35f; // todo: custom modifier in worldserver.conf
...
int32 ArenaTeam::WonAgainst(uint32 Own_MMRating, uint32 Opponent_MMRating, int32& rating_change)
{
    ...
    if (rating_change > 0)
    {
        ...
        if (trans)
        {
            // sLog->outError(LOG_FILTER_SQL, "WINNER: Type: %u, OldRating: %u, Rank: %u, TeamId: %u", Type, OldRating, Stats.Rank, TeamId); // Debug code
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ARENA_TEAM_RANK_I);
            stmt->setUInt8(0, Type);
            stmt->setUInt16(1, OldRating);
            stmt->setUInt32(2, Stats.Rank);
            stmt->setUInt32(3, TeamId);
            trans->Append(stmt);
            CharacterDatabase.CommitTransaction(trans);
        }
        // update in DB and core, so the change is viewable without a restart
        ArenaTeamMgr::ArenaTeamContainer::const_iterator i = sArenaTeamMgr->GetArenaTeamMapBegin();
        for (; i != sArenaTeamMgr->GetArenaTeamMapEnd(); ++i)
        {
            if (i->second->GetType() == Type && i->second->GetStats().Rating > OldRating && i->second->GetStats().Rank >= Stats.Rank && i->second->GetId() != TeamId && i->second->GetStats().Rank != 0)
                i->second->Stats.Rank = i->second->Stats.Rank + 1;
        }
...
int32 ArenaTeam::LostAgainst(uint32 Own_MMRating, uint32 Opponent_MMRating, int32& rating_change)
{
    ...
    if (rating_change < 0)
    {
        ...
        if (trans)
        {
            // sLog->outError(LOG_FILTER_SQL, "LOSER: Type: %u, OldRating: %u, Rank: %u, TeamId: %u", Type, OldRating, Stats.Rank, TeamId); // Debug code
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ARENA_TEAM_RANK_D);
            stmt->setUInt8(0, Type);
            stmt->setUInt16(1, OldRating);
            stmt->setUInt32(2, Stats.Rank);
            stmt->setUInt32(3, TeamId);
