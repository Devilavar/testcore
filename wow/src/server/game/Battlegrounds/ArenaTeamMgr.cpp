...
void ArenaTeamMgr::LoadArenaTeams(uint8 type)
{
    ...
    //                                                        0        1         2         3          4              5            6            7           8
    QueryResult result = CharacterDatabase.PQuery("SELECT arenaTeamId, name, captainGuid, type, backgroundColor, emblemStyle, emblemColor, borderStyle, borderColor, "
    //      9        10        11         12           13       14
        "rating, weekGames, weekWins, seasonGames, seasonWins, rank FROM arena_team WHERE type = %u ORDER BY arenaTeamId ASC", type);

    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 %uv%u arena teams.", type, type);
        return;
    }
    ...
    QueryResult result2 = CharacterDatabase.PQuery(
        ...
        " INNER JOIN arena_team ate USING (arenaTeamId)"
        " LEFT JOIN characters AS c ON atm.guid = c.guid"
        " LEFT JOIN character_arena_stats AS cas ON c.guid = cas.guid AND (cas.slot = 0 AND ate.type = 2 OR cas.slot = 1 AND ate.type = 3 OR cas.slot = 2 AND ate.type = 5) WHERE ate.type = %u"
        " ORDER BY atm.arenateamid ASC", type);
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u %uv%u arena teams in %u ms", count, type, type, GetMSTimeDiffToNow(oldMSTime));
}
