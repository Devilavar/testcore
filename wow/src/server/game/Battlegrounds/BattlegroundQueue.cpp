...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, Battleground* bg, uint32 side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        for (std::map<uint64, PlayerQueueInfo*>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            // send status packet
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, 0, ginfo->ArenaType, player->GetTeam());
...
void BattlegroundQueue::FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id)
{
    if (!bg->isArena())
    {
        int32 hordeFree = bg->GetFreeSlotsForTeam(HORDE, true);
        int32 aliFree   = bg->GetFreeSlotsForTeam(ALLIANCE, true);
        int32 neededPlayers = hordeFree + aliFree;

        FillXPlayersToBG(bracket_id, bg, false); // unused

        if (neededPlayers)
        {
            for (GroupsQueueType::const_iterator citr = m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].begin(); citr != m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].end(); ++citr)
            {
                if ((*citr)->IsInvitedToBGInstanceGUID)
                    continue;

                uint8 numPlayers = (*citr)->Players.size();
                uint32 lowTeam = hordeFree < aliFree ? ALLIANCE : HORDE;
                if ((lowTeam == ALLIANCE) ? numPlayers <= aliFree : numPlayers <= hordeFree)
                {
                    InviteGroupToBG((*citr), bg, lowTeam);
                    break; // process more on next loop
                }
            }
        }
    }
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME - INVITATION_REMIND_TIME, 0, m_ArenaType, player->GetTeam());
            player->GetSession()->SendPacket(&data);
        }
    }
