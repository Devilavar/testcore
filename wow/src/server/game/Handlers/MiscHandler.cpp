...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData)
{
    ...
    if (IS_ITEM_GUID(guid))
    {
        ...
        if (!item)
        {
            TC_LOG_DEBUG("network", "WORLD: HandleGossipSelectOptionOpcode - Item (GUID: %u) not found or you can't interact with it.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else
    ...
    if ((unit && unit->GetCreatureTemplate()->ScriptID != unit->LastUsedScriptID) || (go && go->GetGOInfo()->ScriptId != go->LastUsedScriptID) || (item && item->GetScriptId() != item->LastUsedScriptID))
    {
    ...
    if (!code.empty())
    {
        ...
        else if (item)
        {
            if (!sScriptMgr->OnGossipSelectCode(_player, item, _player->PlayerTalkClass->GetGossipOptionSender(gossipListId), _player->PlayerTalkClass->GetGossipOptionAction(gossipListId), code.c_str()))
                _player->OnGossipSelect((WorldObject*)item, gossipListId, menuId);
        }
        else
        {
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        ...
        uint32 race = target->getOriginalRace();
        if (!(racemask & (1 << race)))
            continue;
    ...
    data.put(4, sWorld->getBoolConfig(CONFIG_DISPLAY_TOTAL_WHO) ? matchcount : displaycount);                              // insert right count, count of matches
...
void WorldSession::HandleReclaimCorpseOpcode(WorldPacket& recvData)
{
    ...
    // prevent resurrect before res timer is finished
    if (time_t(GetPlayer()->GetDeathTime() + GetPlayer()->GetCorpseReclaimDelay(corpse->GetType() == CORPSE_RESURRECTABLE_PVP)) > time_t(time(NULL))) // if predicted res time is ahead of current time
        return;
