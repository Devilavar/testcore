...
void WorldSession::SendListInventory(uint64 vendorGuid, uint32 sort)
{
    ...
    else if (GetPlayer()->GetDistance(vendor->GetPositionX(), vendor->GetPositionY(), vendor->GetPositionZ()) > 7.0f)
    {
    ...
    {
        if (VendorItem const* item = items->GetItem(slot, sort))
        {
            if (ItemTemplate const* itemTemplate = sObjectMgr->GetItemTemplate(item->item))
            {
                if (!(itemTemplate->AllowableClass & _player->getClassMask()) && itemTemplate->Bonding == BIND_WHEN_PICKED_UP && !_player->IsGameMaster() && !(itemTemplate->FlagsCu & ITEM_FLAGS_CU_SKIP_CLASS_CHECK))
                    continue;
                ...
                if (!_player->IsGameMaster() && !leftInStock)
                    continue;
                if (sObjectMgr->IsCustomExtendedCost(item->ExtendedCost))
                {
                    if (GetPlayer()->CheckExtendedCost(item->ExtendedCost, 1) != EQUIP_ERR_OK)
