...
void WorldSession::HandleJoinChannel(WorldPacket& recvPacket)
{
    ...
    if (ChannelMgr* cMgr = ChannelMgr::forTeam(GetPlayer()->GetTeam()))
    {
        ...
        } else ChatHandler(this).SendSysMessage("Invalid channel name.");
    }
}
...
void WorldSession::HandleChannelInvite(WorldPacket& recvPacket)
{
    ...
    if (now - timeLastChInviteCommand < 5)
    {
        ChatHandler(this).SendSysMessage("Action prevented due to spam protection.");
        return;
    }
    else timeLastChInviteCommand = now;
