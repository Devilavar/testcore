...
void WorldSession::SendAuctionHello(uint64 guid, Creature* unit)
{
    ...
    if (GetPlayer()->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_AUCTION_PLAYEDTIME_REQ))
    {
        SendNotification("You cannot use the auction house until you have atleast %u Minutes %u Seconds of playtime.", 
         sWorld->getIntConfig(CONFIG_AUCTION_PLAYEDTIME_REQ) / 60, 
         uint32((sWorld->getIntConfig(CONFIG_AUCTION_PLAYEDTIME_REQ) % 60)));
        return;
    }
