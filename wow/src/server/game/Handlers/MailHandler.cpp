...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (player->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_MAIL_PLAYEDTIME_REQ))
    {
        SendNotification("You cannot use the mailbox until you have atleast %u Minutes %u Seconds of playtime.", 
         sWorld->getIntConfig(CONFIG_MAIL_PLAYEDTIME_REQ) / 60, 
         uint32((sWorld->getIntConfig(CONFIG_MAIL_PLAYEDTIME_REQ) % 60)));
        return;
    }
