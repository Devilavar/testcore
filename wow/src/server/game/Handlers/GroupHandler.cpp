...
void WorldSession::HandleLootMethodOpcode(WorldPacket& recvData)
{
    ...
    if (lootMethod == MASTER_LOOT && !group->IsMember(lootMaster))
        return;
    /********************/
    //   Disable loot change if in combat, Credit: Aramilialon, Kabekiz
    if (GetPlayer()->IsInCombat())
        {
        //ChatHandler(this).SendSysMessage("Cannot change loot mode at this time."); //Chat log notification to group leader.
        SendNotification("Cannot change loot mode at this time."); //Big Red announcement top of screen.
        return;
        }
    // everything's fine, do it
