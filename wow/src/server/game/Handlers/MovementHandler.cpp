...
void WorldSession::HandleMoveTeleportAck(WorldPacket& recvData)
{
    ...
    if (old_zone != newzone)
    {
        ...
        // in friendly area
            ...
            if (GetPlayer()->pvpInfo.IsInNoPvPArea)
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    // Don't process data when we're being controlled by the server
    if (plrMover && plrMover->HasUnitMovementFlag(MOVEMENTFLAG_SPLINE_ENABLED))
    {
        recvData.rfinish();
        return;
    }
    ...
    PlayerMovementChecker *movChecker = _player->GetMovementChecker();

    if (sWorld->getIntConfig(CONFIG_MOVEMENT_CHECKS_MASK) != 0)
        movChecker->prepare(&movementInfo, opcode);
    ...
    if (sWorld->getIntConfig(CONFIG_MOVEMENT_CHECKS_MASK) != 0)
        if(!movChecker->canMove())
        {
            movChecker->doMovementCorrection();
            return;
        }
