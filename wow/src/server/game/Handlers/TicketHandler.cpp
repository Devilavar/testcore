...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket& recvData)
{
    ...
    if (GetPlayer()->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_TICKET_PLAYEDTIME_REQ))
    {
        SendNotification("You cannot make a ticket until you have atleast %u Minutes %u Seconds of playtime.", 
         sWorld->getIntConfig(CONFIG_TICKET_PLAYEDTIME_REQ) / 60, 
         uint32((sWorld->getIntConfig(CONFIG_TICKET_PLAYEDTIME_REQ) % 60)));
        return;
    }
