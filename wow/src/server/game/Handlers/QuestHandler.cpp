...
#define CLOSE_GOSSIP_CLEAR_DIVIDER() \
    ...
    uint32 PendingSharedQuestId = _player->GetPendingSharedQuestId();

    // player can't accept a shared quest that hasn't been shared to them
    if (PendingSharedQuestId != questId && PendingSharedQuestId != 0)
    {
        _player->SetPendingSharedQuestId(0);
        return;
    }

    _player->SetPendingSharedQuestId(0); // reset value, will be set again if someone shares a quest
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        {
            if (!_player->AddQuestAndCheckCompletion(quest, object))
                return;
            ...
            if (quest->HasFlag(QUEST_FLAGS_PARTY_ACCEPT))
            {
                if (Group* group = _player->GetGroup())
                {
                    for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
                    {
                        ...
                        player->SetPendingSharedQuestId(questId); // set the quest we are sharing in stored player data, to prevent manipulation of packet
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        uint64 objguid = object->GetGUID();

        if (quest->IsAutoAccept() && _player->CanAddQuest(quest, true))
            _player->AddQuestAndCheckCompletion(quest, object);

        if (quest->HasFlag(QUEST_FLAGS_AUTOCOMPLETE))
            _player->PlayerTalkClass->SendQuestGiverRequestItems(quest, objguid, _player->CanCompleteQuest(quest->GetQuestId()), true);
        else
            _player->PlayerTalkClass->SendQuestGiverQuestDetails(quest, objguid, true);
    }
}
