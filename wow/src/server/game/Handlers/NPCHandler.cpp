...
void WorldSession::HandleGossipHelloOpcode(WorldPacket& recvData)
{
    ...
    else if (GetPlayer()->GetDistance(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ()) > 7.0f) // actual dist limit is 5.5
    {
        TC_LOG_ERROR("network", "[%s] tried to interact with a vendor that is not in range", GetPlayerInfo().c_str());
        return;
    }
...
{
    TC_LOG_ERROR("network", "WORLD: CMSG_BINDER_ACTIVATE");
    uint64 npcGUID;
    recvData >> npcGUID;
    ...
    if (!unit)
    {
        TC_LOG_ERROR("network", "WORLD: CMSG_BINDER_ACTIVATE FAILED");
...
void WorldSession::SendBindPoint(Creature* npc)
{
    ...
    homeLoc = npc->GetWorldLocation();

    _player->SetHomebind(homeLoc, areaId);
    ...
    TC_LOG_DEBUG("entities.player", "Player %u has new homebind X: %f, Y: %f, Z: %f, MapId: %u, AreaId: %u",
        _player->GetGUIDLow(), homeLoc.GetPositionX(), homeLoc.GetPositionY(), homeLoc.GetPositionZ(), homeLoc.GetMapId(), areaId);
