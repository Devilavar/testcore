...
void WorldSession::HandlePetAction(WorldPacket& recvData)
{
    ...
    for (Unit::ControlList::iterator itr = GetPlayer()->m_Controlled.begin(); itr != GetPlayer()->m_Controlled.end(); ++itr)
        if (((*itr)->GetEntry() == pet->GetEntry() || (*itr)->GetEntry() == 31216 || (*itr)->GetEntry() == 27829) && (*itr)->IsAlive()) // Gargoyle and Mirror Images are special cases
...
void WorldSession::HandlePetActionHelper(Unit* pet, uint64 guid1, uint32 spellid, uint16 flag, uint64 guid2)
{
    ...
    if (pet->GetEntry() == 27829)
        if (!pet->GetOwner()->HasAura(61777)) // Summon Gargoyle-- only applied when gargoyle is active
            return;

    // AI is locked when taunted, but can cast spells & switch react state
    if (pet->GetVictim() && charmInfo->IsLockedTarget())
        if (flag != ACT_ENABLED && flag != COMMAND_ABANDON && flag != ACT_REACTION)
            return;
        ...
        case ACT_COMMAND:                                   //0x07
            switch (spellid)
            {
                case COMMAND_STAY:                          //flat=1792  //STAY
                    if (!pet->HasUnitState(UNIT_STATE_LOST_CONTROL))
                    {
                        pet->StopMoving();
            ...
            if (result == SPELL_CAST_OK)
            {
                ...
                if (unit_target && !GetPlayer()->IsFriendlyTo(unit_target) && !pet->isPossessed() && !pet->IsVehicle())
                {
                    ...
                    if (pet->GetVictim() != unit_target)
                    {
                        if (pet->GetVictim())
                            pet->AttackStop();
                        pet->GetMotionMaster()->Clear(true, MOTION_SLOT_ACTIVE);
