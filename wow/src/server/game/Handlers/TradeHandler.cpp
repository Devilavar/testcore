...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    if (GetPlayer()->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_TRADE_PLAYEDTIME_REQ))
    {
        SendNotification("You cannot trade until you have atleast %u Minutes %u Seconds of playtime.", 
         sWorld->getIntConfig(CONFIG_TRADE_PLAYEDTIME_REQ) / 60, 
         uint32((sWorld->getIntConfig(CONFIG_TRADE_PLAYEDTIME_REQ) % 60)));
        return;
    }
