    ...
    switch (cond->SourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                TC_LOG_ERROR("sql.sql", "SourceEntry %u SourceGroup %u in `condition` table - spell %u does not have implicit targets of types: _AREA_, _CONE_, _NEARBY_ for effect %u, SourceGroup should be %u, ignoring.", cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i), cond->SourceGroup);
            }
