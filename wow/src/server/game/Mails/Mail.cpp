...
void WorldSession::SendExternalMails()
{
    ...
    if(result)
    {
        do
        {
           ...
           Player *receiver = sObjectAccessor->FindPlayer(receiver_guid);

            // set some vars
            uint32 errors = 0;
            uint32 max_money = 2000000000;

            // check to see if player exists
            QueryResult check_player_exists = CharacterDatabase.PQuery("SELECT 1 FROM characters WHERE guid = '%u'", receiver_guid);
            ...
            else
            {
                if(receiver != 0)
                {
                    // make sure money doesn't exceed max value
                    if(money > max_money)
                        money = max_money;

                    // check to see if item exists
                    ItemTemplate const * check_item_exists = sObjectMgr->GetItemTemplate(ItemID);
                    if(!check_item_exists)
                        ItemID = 0;

                    if(ItemID != 0)
                    {
                        // make sure item count is greater than 0 or it will crash
                        if(ItemCount == 0)
                            ItemCount = 1;
                    ...
                    // if there's no errors, update the row
                    if(errors == 0)
                        CharacterDatabase.PExecute("UPDATE mail_external SET sent = '" UI64FMTD "' WHERE id = '%u'", uint64(time(NULL)), id);
                }
            }
        }
        // next row
