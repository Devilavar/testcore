...
void BattlefieldWG::OnBattleEnd(bool endByTimer)
{
    ...
    for (GuidSet::const_iterator itr = m_PlayersInWar[GetDefenderTeam()].begin(); itr != m_PlayersInWar[GetDefenderTeam()].end(); ++itr)
    {
        if (Player* player = sObjectAccessor->FindPlayer(*itr))
        {
            ...
            // custom reward
            uint32 item = sWorld->getIntConfig(CONFIG_WINTERGRASP_CUSTOMREWARD_ITEM);
            uint32 count = sWorld->getIntConfig(CONFIG_WINTERGRASP_CUSTOMREWARD_AMOUNT);
            if (count && item)
...
void BattlefieldWG::HandleKill(Player* killer, Unit* victim)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        DoCompleteOrIncrementAchievement(ACHIEVEMENT_CRITERIA_TYPE_SPECIAL_PVP_KILL, killer);
...
{
    if ((player->GetZoneId() != ICECROWN) && (player->GetZoneId() != WINTERGRASP) && (player->GetZoneId() != SHOLAZAR) && (player->GetZoneId() != DRAGONBLIGHT)) // hacky way of preventing players from exploiting the join battle que, due to opcode error
    {
