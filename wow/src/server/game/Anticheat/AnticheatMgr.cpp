...
void AnticheatMgr::JumpHackDetection(Player* player, MovementInfo movementInfo,uint32 opcode)
{
    ...
    if (m_Players[key].GetLastOpcode() == MSG_MOVE_JUMP && opcode == MSG_MOVE_JUMP)
    {
        ...
        TC_LOG_INFO("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Detected for jump-hacking", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
    }
}
...
void AnticheatMgr::WalkOnWaterHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    // ako ne e gm da blokira hakera
    if (!player->IsGameMaster())
         return;
         
    if (player->HasAura(16593) || player->HasAura(16595) || player->HasAura(16591))
        return;
    ...
    TC_LOG_INFO("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Detected for waterwalk-hacking.", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
...
void AnticheatMgr::FlyHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    TC_LOG_INFO("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Detected for fly-hacking", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
...
void AnticheatMgr::TeleportPlaneHackDetection(Player* player, MovementInfo movementInfo)
{
    ...
    if (z_diff > 1.0f)
    {
        TC_LOG_INFO("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Detected for teleport-hacking.", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
...
void AnticheatMgr::ClimbHackDetection(Player *player, MovementInfo movementInfo, uint32 opcode)
{
    ...
    {
        TC_LOG_WARN("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Detected for wall-hacking.", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
...
void AnticheatMgr::SpeedHackDetection(Player* player,MovementInfo movementInfo)
{
    ...
    if (clientSpeedRate > speedRate)
    {
        ...
        TC_LOG_INFO("anticheat", "PASSIVEANTICHEAT: [Player: %s (Guid: %u, Account: %u)] Detected for speed-hacking", player->GetName().c_str(), player->GetGUIDLow(), player->GetSession()->GetAccountId());
    }
}
...
void AnticheatMgr::BuildReport(Player* player,uint8 reportType)
{
	...
	if (!player->IsGameMaster())
		if (m_Players[key].GetTotalReports() >= sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_AMOUNT))
		{
