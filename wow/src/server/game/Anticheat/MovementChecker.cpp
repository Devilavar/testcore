...
bool PlayerMovementChecker::isSpeedOk()
{
    ...
    uint32 clientTimeDelta = _movementInfo->time - _lastClientTime + (_latency * 3); // default @ 2
    const uint32 serverTimeDelta = _currServerTime - _lastServerTime + _latency + _timediff;

    if(clientTimeDelta > 1500) // default @ 1k
    ...
    if(_mover->HasUnitState(UNIT_STATE_CHARGING))
        return true;
...
void PlayerMovementChecker::SendGMWarning(uint32 type)
{
    ...
    TC_LOG_WARN("anticheat", "ACTIVEANTICHEAT:  [Player: %s (Guid: %u, Account: %u)] caught %s hacking", playername.c_str(), _player->GetGUIDLow(), _player->GetSession()->GetAccountId(), _type.c_str());
