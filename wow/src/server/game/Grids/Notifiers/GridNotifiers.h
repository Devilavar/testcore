...
namespace Trinity
{
    ...
    */
    ...
    {
        public:
            explicit NearestHostileUnitInAttackDistanceCheck(Creature const* creature, float dist = 0, bool calcAggroRange = false, bool CheckLoS = false) : me(creature)
            {
                m_range = calcAggroRange ? MAX_AGGRO_RADIUS : (dist == 0 ? 9999 : dist);
                m_force = calcAggroRange ? true : (dist == 0 ? false : true);
                m_calcAggroRange = calcAggroRange;
                m_CheckLoS = CheckLoS;
                closest = 0.0f;
            }
            ...
            {
                if (m_calcAggroRange)
                    m_range = me->GetAggroRange(u);

                if (closest && !me->IsWithinDistInMap(u, closest))
                    return false;
                ...
                if (m_CheckLoS && !u->IsWithinLOSInMap(me))
                    return false;
                ...
                closest = me->GetDistance(u);   // use found unit range as new range limit for next check
                return true;
            }
