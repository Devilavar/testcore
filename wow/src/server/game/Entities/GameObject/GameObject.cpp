        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (goInfo->trap.type == 2 && !goInfo->trap.diameter)
                    {
...
void GameObject::UseDoorOrButton(uint32 time_to_restore, bool alternative /* = false */, Unit* user /*=NULL*/)
{
    ...
    if (!time_to_restore && user && user->ToPlayer() && user->ToPlayer()->IsGameMaster())
        time_to_restore = 1800; // 30 min
...
void GameObject::SetLootState(LootState state, Unit* unit)
{
    ...
    if (m_model)
    {
        ...
        switch (GetGoType())
        {
            // doors and generic shouldn't be handled here
            case GAMEOBJECT_TYPE_DOOR:
            case GAMEOBJECT_TYPE_GENERIC:
                switch (state)
                {
                    case GO_ACTIVATED:
                    case GO_READY:
                        SetGoState(state == GO_ACTIVATED ? GO_STATE_ACTIVE : GO_STATE_READY);
                        break;
                    default:
                        SetGoState(GetGoState());
                }
                break;
            default:
