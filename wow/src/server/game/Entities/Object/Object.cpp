...
bool WorldObject::IsWithinLOSInMap(const WorldObject* obj) const
{
    ...
    // HACK for ice tomb's not being attackable (LoS)
    if (obj->GetTypeId() == TYPEID_UNIT || GetTypeId() == TYPEID_UNIT)
        if (obj->GetEntry() == 36980 || GetEntry() == 36980)
            return true;
...
void WorldObject::UpdateAllowedPositionZ(float x, float y, float &z) const
{
    ...
    switch (GetTypeId())
    {
        case TYPEID_UNIT:
        {
            ...
            if (!ToCreature()->CanFly() && !ToCreature()->HasUnitMovementFlag(MOVEMENTFLAG_CAN_FLY))
            {
...
// select nearest unit within the given distance
Unit* WorldObject::SelectNearestTarget(float dist, bool playerOnly /* = false */) const
{
    CellCoord p(Trinity::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();
    ...
    if (!playerOnly)
    {
        ...
        Trinity::UnitLastSearcher<Trinity::AnyUnitInObjectRangeCheck> searcher(this, target, u_check);

        TypeContainerVisitor<Trinity::UnitLastSearcher<Trinity::AnyUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
        TypeContainerVisitor<Trinity::UnitLastSearcher<Trinity::AnyUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
    ...
    if (!target)
    {
        Trinity::NearestPlayerInObjectRangeCheck checker(this, dist);
        Trinity::PlayerLastSearcher<Trinity::NearestPlayerInObjectRangeCheck> searcher(this, plrTarget, checker);
        VisitNearbyObject(dist, searcher);

        return (Unit*)plrTarget;
    }
