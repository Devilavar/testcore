...
struct VendorItem
{
    VendorItem(uint32 _item, int32 _maxcount, uint32 _incrtime, uint32 _ExtendedCost, uint32 _sort)
        : item(_item), maxcount(_maxcount), incrtime(_incrtime), ExtendedCost(_ExtendedCost), sort(_sort) { }
...
struct VendorItemData
{
    ...
    VendorItem* GetItem(uint32 slot, uint32 sort = 0) const
    {
        if (slot >= m_items.size())
            return NULL;

        if (m_items[slot] && m_items[slot]->sort != sort)
            return NULL;
    ...
    void AddItem(uint32 item, int32 maxcount, uint32 ptime, uint32 ExtendedCost, uint32 sort)
    {
        m_items.push_back(new VendorItem(item, maxcount, ptime, ExtendedCost, sort));
    }
    bool RemoveItem(uint32 item_id, uint32 sort);
    VendorItem const* FindItemCostPair(uint32 item_id, uint32 extendedCost, uint32 sort) const;
    void Clear()
    {
        for (VendorItemList::const_iterator itr = m_items.begin(); itr != m_items.end(); ++itr)
            delete (*itr);
...
class Creature : public Unit, public GridObject<Creature>, public MapObject
{
    public:
        ...
        bool CanTrain(Player* player, bool msg) const;
        ...
        Unit* SelectNearestTargetInAttackDistance(float dist = 0, bool CalcAggroRange = false, bool CheckLoS = false) const;
        ...
        bool IsAtHome() { return m_homePosition.GetExactDist(m_positionX, m_positionY, m_positionZ) <= 0.5f; }
        ...
        Unit* SelectVictim(bool allowEvade = true);
