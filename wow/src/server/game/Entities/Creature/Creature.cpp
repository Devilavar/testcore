...
bool VendorItemData::RemoveItem(uint32 item_id, uint32 sort)
{
    bool found = false;
    for (VendorItemList::iterator i = m_items.begin(); i != m_items.end();)
    {
        if ((*i)->item == item_id && (*i)->sort == sort)
        {
            i = m_items.erase(i++);
            found = true;
        }
...
VendorItem const* VendorItemData::FindItemCostPair(uint32 item_id, uint32 extendedCost, uint32 sort) const
{
    for (VendorItemList::const_iterator i = m_items.begin(); i != m_items.end(); ++i)
        if ((*i)->item == item_id && (*i)->ExtendedCost == extendedCost && (*i)->sort == sort)
            return *i;
    return NULL;
}
...
bool Creature::CanTrain(Player* player, bool msg) const
{
    if (!IsTrainer())
        return false;
    ...
    {
        TC_LOG_ERROR("sql.sq;", "Creature %u (Entry: %u) have UNIT_NPC_FLAG_TRAINER but have empty trainer spell list.",
            GetGUIDLow(), GetEntry());
        return false;
    }
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist, bool calcAggroRange, bool CheckLoS) const
{
    CellCoord p(Trinity::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();
    ...
    {
        Trinity::NearestHostileUnitInAttackDistanceCheck u_check(this, dist, calcAggroRange, CheckLoS);
