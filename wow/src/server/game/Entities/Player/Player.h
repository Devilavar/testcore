...
#include "SpellMgr.h"
#include "Unit.h"
#include "../Anticheat/MovementChecker.h"
...
struct DuelInfo
{
    DuelInfo() : initiator(NULL), opponent(NULL), startTimer(0), startTime(0), outOfBound(0), isMounted(false), opponentHealthPct(0) {}
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        bool AddQuestAndCheckCompletion(Quest const* quest, Object* questGiver);
        bool AddQuest(Quest const* quest, Object* questGiver);
        ...
        bool CanTransmogItem(const ItemTemplate* vItemTemplate, const ItemTemplate* pItemTemplate, bool action);
        ...
        int16 GetCombatRating(CombatRating cr) const { ASSERT(cr < MAX_COMBAT_RATING); return m_baseRatingValue[cr]; }
        ...
        std::string CreateExtendedCostErrorMessage(uint32 extendedCost);
        InventoryResult CheckExtendedCost(ItemExtendedCostEntry const* cost, uint8 count);
        InventoryResult CheckExtendedCost(uint32 ExtendedCost, uint8 count);
        ...
        uint32 GetPendingSharedQuestId() { return m_PendingSharedQuestId; }
        void SetPendingSharedQuestId(uint32 PendingSharedQuestId) { m_PendingSharedQuestId = PendingSharedQuestId; }

        time_t const& GetDeathTime() const { return m_deathTime; }
        ...
        void StartOrPauseFreezeStunTimers(bool start)
        {
                start ? pauseTimerChecks = false : pauseTimerChecks = true;
        }
        void SetFreezeStunTimer(bool freeze, uint32 _timer)
        {
            freeze ? freezeTimer = _timer : stunTimer = _timer;
            pauseTimerChecks = false;
        }
        ...
        bool HasOfflineTitle(WorldSession* session, uint32 playerguid, uint32 id);
        void SetTitle(CharTitlesEntry const* title, bool lost = false);
        void SetOfflineTitle(WorldSession* session, uint32 playerguid, uint32 titleid);
        ...
        void SetFakeRace() { m_FakeRace = GetOTeam() == ALLIANCE ? RACE_BLOODELF : RACE_HUMAN; }
    ...
    protected:
        ...
        */
        ...
        void _LoadAccountMessages(QueryResult result);
        ...
        typedef std::list<std::string> AccountMessageList;
        AccountMessageList m_accountmessages;
