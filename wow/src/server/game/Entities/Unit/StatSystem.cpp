...
bool Guardian::UpdateAllStats()
{
    ...
    {
        float RatingChange = GetOwner()->ToPlayer()->GetCombatRating(CR_HASTE_MELEE) * GetOwner()->ToPlayer()->GetRatingMultiplier(CR_HASTE_MELEE);
        for (uint8 i = 0; i < MAX_ATTACK; ++i)
            UpdateHaste(WeaponAttackType(i), RatingChange, true);

        float ModSpeed = float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_MELEE_SLOW)) * 100.0f); // SPELL_AURA_MELEE_SLOW also affects casting speed
        float ModSpeedAll = float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_MOD_SPEED_SLOW_ALL)) * 100.0f); // SPELL_AURA_MOD_SPEED_SLOW_ALL also affects casting speed

        for (uint8 i = 0; i < MAX_ATTACK; ++i)
        {
            UpdateHaste(WeaponAttackType(i), -ModSpeed, true);
            UpdateHaste(WeaponAttackType(i), -ModSpeedAll, true);
            UpdateHaste(WeaponAttackType(i), -(float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_MOD_MELEE_RANGED_HASTE)) * 100.0f)), true);
            UpdateHaste(WeaponAttackType(i), -(float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_MOD_ATTACKSPEED)) * 100.0f)), true);
            UpdateHaste(WeaponAttackType(i), -(float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_MOD_MELEE_HASTE)) * 100.0f)), true);
        }

        UpdateHaste(MAX_ATTACK, -ModSpeed, true);
        UpdateHaste(MAX_ATTACK, -ModSpeedAll, true);
        UpdateHaste(MAX_ATTACK, -(float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK)) * 100.0f)), true);
        UpdateHaste(MAX_ATTACK, -(float((1.0f - GetOwner()->GetTotalAuraMultiplier(SPELL_AURA_HASTE_SPELLS)) * 100.0f)), true);
    }
...
void Guardian::UpdateHaste(WeaponAttackType att, float amount, bool apply)
{
    Player* owner = GetOwner()->ToPlayer();
    if (!owner)
        return;
        ...
        switch (att)
        {
            case BASE_ATTACK:
                if (owner->getClass() == CLASS_DEATH_KNIGHT)
                    ApplyCastTimePercentMod(amount, apply); // Death Knights pets inhereit haste from owners base attack
                // no break
            case OFF_ATTACK:
            case RANGED_ATTACK:
                ApplyAttackTimePercentMod(att, amount, apply);
                break;
            default:
                if (owner->getClass() != CLASS_DEATH_KNIGHT)
                    ApplyCastTimePercentMod(amount, apply);
        }
    }
}
