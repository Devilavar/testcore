...
enum UnitState
{
    ...
    UNIT_STATE_JUMPING_MOVE    = 0x00080000,
    ...
    UNIT_STATE_CONTROLLED      = (UNIT_STATE_CONFUSED | UNIT_STATE_STUNNED | UNIT_STATE_FLEEING | UNIT_STATE_JUMPING_MOVE),
    UNIT_STATE_LOST_CONTROL    = (UNIT_STATE_CONTROLLED | UNIT_STATE_CHARGING),
...
struct CharmInfo
{
    public:
        ...
        void HandleTaunt(bool apply, bool init = false);
        void SetIsLockedTarget(bool val);
        bool IsLockedTarget();
...
class Unit : public WorldObject
{
    public:
        ...
        bool AttackStop(bool switchtarget = false);
        ...
        float CalcDebuffResistance(Unit* target, SpellInfo const* procSpell);
        ...
        bool CanHaveThreatWith(Unit const* target) const;
        bool _CanHaveThreatWith(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj = NULL) const;
        ...
        bool HasPositiveAuraWithInterruptFlag(uint32 flag, uint64 guid = 0) const;
        ...
        uint32 SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack = 1, Aura* base = NULL, bool init = false) const;
        ...
        float CalcPeriodicCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType = BASE_ATTACK) const;
        ...
        void CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo, int32 calc_resist);
        ...
        void AfterDamageCalc(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, SpellInfo const* spellInfo = NULL);
        ...
        bool IsInAir(float x = 0.0f, float y = 0.0f, float z = 0.0f, float height = 4.0f, bool vmap = true) const;
