...
void Unit::Update(uint32 p_time)
{
    ...
    {
        m_HostileRefManager.DeleteInvalidReferences();
...
{
    static uint32 const positionUpdateDelay = 50;
...
void Unit::CalculateMeleeDamage(Unit* victim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (int32(damageInfo->damage) > 0)
    {
        ...
        CalcAbsorbResist(damageInfo->target, SpellSchoolMask(damageInfo->damageSchoolMask), DIRECT_DAMAGE, damageInfo->damage, &damageInfo->absorb, &damageInfo->resist, NULL, -1);
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo, int32 calc_resist)
{
    ...
    bool binary = (spellInfo && (spellInfo->AttributesCu & SPELL_ATTR0_CU_BINARY));
    if (!binary)
        dmgInfo.ResistDamage((damage * (calc_resist >= 0 ? calc_resist : CalcSpellResistance(victim, schoolMask, binary, spellInfo))) / 100);

    // Ignore Absorption Auras
    float auraAbsorbMod = 0;
    float ignoreAbsorbMod = 0.0f;
    ...
    for (AuraEffectList::const_iterator itr = AbsIgnoreAurasA.begin(); itr != AbsIgnoreAurasA.end(); ++itr)
    {
        ...
        if ((*itr)->GetAmount() > auraAbsorbMod)
            ignoreAbsorbMod += float((*itr)->GetAmount());
    }

    RoundToInterval(ignoreAbsorbMod, 0.0f, 100.0f);
    ...
    if (victim != this)
    {
        ...
        for (AuraEffectList::iterator itr = vSplitDamageFlatCopy.begin(); (itr != vSplitDamageFlatCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
        {
            ...
            {
                victim->RemoveAura((*itr)->GetBase()->GetId(), caster ? caster->GetGUID() : NULL);
                continue;
            }
        ...
        for (AuraEffectList::iterator itr = vSplitDamagePctCopy.begin(); itr != vSplitDamagePctCopy.end() && dmgInfo.GetDamage() > 0; ++itr)
        {
            ...
            if (!caster || (caster == victim) || !caster->IsInWorld() || !caster->IsAlive())
            {
                victim->RemoveAura(aurApp->GetBase()->GetId(), caster ? caster->GetGUID() : NULL);
                continue;
            }
    ...
    // Roar of Sacrifice - hack for now since this is the only (known) spell with this behavior
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        if (Aura* aura = victim->GetAura(53480))
            if (Unit* caster = aura->GetCaster())
                if (caster->IsPet())
                    DealDamage(caster, (damage - (dmgInfo.GetResist() + dmgInfo.GetAbsorb())) * 0.2f, NULL, damagetype);
    }

    *resist = dmgInfo.GetResist();
    *absorb = dmgInfo.GetAbsorb() - (dmgInfo.GetAbsorb() * (ignoreAbsorbMod / 100.0f));
}
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    if ((attType == BASE_ATTACK || attType == OFF_ATTACK) && (!IsWithinLOSInMap(victim) && GetTypeId() != TYPEID_UNIT))
        return;
...
uint32 Unit::CalcMagicSpellHitChance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo)
{
    ...
    if (leveldif < 3)
    {
        if (GetTypeId() == TYPEID_UNIT && ToCreature()->isWorldBoss())
            return hit;
    ...
    modHitChance -= CalcDebuffResistance(victim, spellInfo);

    hit = modHitChance * 100;
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (dummySpell->SpellIconID == 1697)
            {
                ...
                if (procSpell == 0 || !(procEx & (PROC_EX_NORMAL_HIT|PROC_EX_CRITICAL_HIT)) || this == victim || (procEx & PROC_EX_INTERNAL_DOT))
                    return false;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49028)
            {
                ...
                if (pPet && pPet->GetVictim() && damage && procSpell)
                {
                    ...
                    pPet->CastCustomSpell(pPet->GetVictim(), procSpell->Id, &procDmg, 0, 0, true);
                    break;
                }
...
bool Unit::AttackStop(bool switchtarget)
{
    if (!m_attacking)
        return false;

    // Pet's with AI should interrupt casts and return when told to stop attacking, unless they are switching targets
    if (GetTypeId() == TYPEID_UNIT)
    {
        if (Pet* pet = ToPet())
        {
    ...
    for (Unit::AuraEffectList::const_iterator itr = magnetAuras.begin(); itr != magnetAuras.end(); ++itr)
    {
        if (Unit* magnet = (*itr)->GetBase()->GetCaster())
            if (spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK
                && spellInfo->CheckTarget(this, magnet, false, true) == SPELL_CAST_OK
...
void Unit::RemoveAllControlled()
{
    ...
    while (!m_Posessing.empty())
    {
        Unit* target = *m_Posessing.begin();
        m_Posessing.erase(m_Posessing.begin());
        if (target->GetCharmerGUID() == GetGUID())
            target->RemoveCharmAuras();
        else if (target->GetOwnerGUID() == GetGUID() && target->IsSummon())
            target->ToTempSummon()->UnSummon();
...
float Unit::CalcPeriodicCrit(Unit* victim, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack, Aura* base, bool init) const
{
    ...
    if (!(spellProto->AttributesEx6 & SPELL_ATTR6_NO_DONE_PCT_DAMAGE_MODS) && !spellProto->IsRankOf(sSpellMgr->GetSpellInfo(12162)))
    {
        ...
        {
            if ((spellProto->EquippedItemClass == -1 && (*i)->GetSpellInfo()->EquippedItemClass != -1) && !((*i)->GetSpellInfo()->AttributesEx5 & SPELL_ATTR5_SPECIAL_ITEM_CLASS_CHECK))    //prevent apply mods from weapon specific case to non weapon specific spells (Example: thunder clap and two-handed weapon specialization)
                continue;
    ...
    switch (spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x00004000)
                if (victim != this && !victim->HealthAbovePct(25))
                    DoneTotalMod *= 4;
    ...
    if (damagetype == DOT && base)
    {
        if (init)
        {
            base->SetDmgPctModifer(DoneTotalMod);
            base->SetCritModifier(CalcPeriodicCrit(victim, spellProto, spellProto->GetSchoolMask()));
        }

        DoneTotalMod = base->GetDmgPctModifier();
    }
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    {
        switch ((*i)->GetSpellInfo()->SpellFamilyName)
        {
            case SPELLFAMILY_ROGUE:
            {
...
bool Unit::IsImmunedToSpell(SpellInfo const* spellInfo) const
{
    ...
    if (GetTypeId() == TYPEID_UNIT)
        if (spellInfo->Id == 70539 && GetEntry() != 36678) // Abomination slow spellid, professor putricide entry
            return false;
...
bool Unit::CanHaveThreatWith(Unit const* target) const
{
    return _CanHaveThreatWith(target, NULL);
}

bool Unit::_CanHaveThreatWith(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ASSERT(target);
    ...
    // can't attack unattackable units or GMs
    if (plrTarget && (target->HasUnitState(UNIT_STATE_UNATTACKABLE) || target->ToPlayer()->IsGameMaster()))
        return false;
    ...
    // can't attack untargetable
    if (plrTarget && (!bySpell || !(bySpell->AttributesEx6 & SPELL_ATTR6_CAN_TARGET_UNTARGETABLE)) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE))
        return false;
    ...
    // check flags
    if ((plrTarget && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_TAXI_FLIGHT | UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_UNK_16))
        || (!HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC) && GetTypeId() == TYPEID_UNIT)
        || (!target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC) && !plrTarget)
        || (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC) && plrTarget)
        // check if this is a world trigger cast - GOs are using world triggers to cast their spells, so we need to ignore their immunity flag here, this is a temp workaround, needs removal when go cast is implemented properly
        || (GetEntry() != WORLD_TRIGGER && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE) && HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC)))
        return false;
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
        {
            ...
            if (GetTypeId() == TYPEID_UNIT)
            {
                base_speed = ToCreature()->GetCreatureTemplate()->speed_run;
                speed *= base_speed; // at this point, MOVE_WALK is never reached
            }
    ...
    if (slow)
    {
        ...
        {
            float minSpeed = (base_speed * (minSpeedMod / 100.0f));
            if (minSpeed > speed)
                speed = minSpeed;
        }
    }
...
Unit* Creature::SelectVictim(bool allowEvade)
{
    ...
    if (target && _IsTargetAcceptable(target) && CanCreatureAttack(target) && !((IsPet() || IsGuardian()) && target->HasBreakableByDamageCrowdControlAura()))
    {
        SetInFront(target);
        return target;
    }
    ...
    if (!iAuras.empty())
    {
        ...
        {
            if ((*itr)->GetBase()->IsPermanent() && (allowEvade && (!IsAtHome() || !m_ThreatManager.isThreatListEmpty())))
            {
                AI()->EnterEvadeMode();
                break;
            }
    ...
    if (allowEvade && (!IsAtHome() || !m_ThreatManager.isThreatListEmpty()))
        AI()->EnterEvadeMode();
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (!(spellProto->Attributes & (SPELL_ATTR0_ABILITY|SPELL_ATTR0_TRADESPELL)) && !(spellProto->AttributesEx6 & SPELL_ATTR6_UNK17) && ((GetTypeId() == TYPEID_PLAYER && spellProto->SpellFamilyName) || GetTypeId() == TYPEID_UNIT))
        ...
        castTime = 500;
    else if(spellProto->Effects[0].Effect == SPELL_EFFECT_ENCHANT_ITEM || spellProto->Effects[0].Effect == SPELL_EFFECT_APPLY_GLYPH || spellProto->Effects[0].Effect == SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC)
        castTime = 1;
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (Unit* owner = GetOwner())
        {
            ...
            if (owner->m_Posessing.find(this) != owner->m_Posessing.end())
            {
                TC_LOG_FATAL("entities.unit", "Unit %u is in posessing list of %u when removed from world", GetEntry(), owner->GetEntry());
                ASSERT(false);
            }
        }
...
: _unit(unit), _CommandState(COMMAND_FOLLOW), _petnumber(0), _barInit(false), _oldReactState(REACT_PASSIVE),
  ...
  _stayX(0.0f), _stayY(0.0f), _stayZ(0.0f), _isLockedTarget(false)
{
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        if (spellProto->HasAura(SPELL_AURA_MOD_STEALTH) && isVictim && procExtra & PROC_EX_ABSORB)
            active = true;

        if (procExtra & PROC_EX_ABSORB && (!procAura || (procAura && procAura->DmgClass & SPELL_DAMAGE_CLASS_MELEE)))
        {
            switch (spellProto->SpellFamilyName)
            {
                case SPELLFAMILY_HUNTER:
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_MOD_STEALTH:
                            if (procExtra & PROC_EX_ABSORB)
                            {
                                if (!procAura)
                                    takeCharges = true;
                                else if (procAura->CalcCastTime() || procAura->IsChanneled() || !procAura->IsDstSpell())
                                    takeCharges = true;
                            }
                            else takeCharges = true;
                        break;
...
float Unit::CalcDebuffResistance(Unit* target, SpellInfo const* procSpell)
{
    if (!target->HasAuraType(SPELL_AURA_MOD_DEBUFF_RESISTANCE))
        return 0.0f;

    float missChance = 0.0f;
    if (procSpell->Dispel == DISPEL_POISON || procSpell->Dispel == DISPEL_DISEASE || procSpell->Dispel == DISPEL_MAGIC)
    {
        missChance += float(target->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(procSpell->Dispel)));
        missChance += float(target->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(procSpell->Dispel)));
    }
    else
    {
        DamageInfo damageInfo = DamageInfo(this, target, 0, procSpell, procSpell ? SpellSchoolMask(procSpell->SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE);
        HealInfo healInfo = HealInfo(0);
        ProcEventInfo eventInfo = ProcEventInfo(this, target, target, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, 0, 0, PROC_EX_NORMAL_HIT, NULL, &damageInfo, &healInfo);

        ProcTriggeredList procTriggered;
        // Fill procTriggered list
        for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr != GetAppliedAuras().end(); ++itr)
        {
            if (procSpell->Id == itr->first)
                continue;
            ProcTriggeredData triggerData(itr->second->GetBase());

            SpellInfo const* spellProto = itr->second->GetBase()->GetSpellInfo();

            if (!IsTriggeredAtSpellProcEvent(target, triggerData.aura, procSpell, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_NORMAL_HIT, BASE_ATTACK, false, true, triggerData.spellProcEvent))
                continue;

            // do checks using conditions table
            ConditionList conditions = sConditionMgr->GetConditionsForNotGroupedEntry(CONDITION_SOURCE_TYPE_SPELL_PROC, spellProto->Id);
            ConditionSourceInfo condInfo = ConditionSourceInfo(this, target);
            if (!sConditionMgr->IsObjectMeetToConditions(condInfo, conditions))
                continue;
        ...
        if (!procTriggered.empty())
        {
            for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
            {
                ...
                {
                    AuraEffect* triggeredByAura = i->aura->GetEffect(effIndex);
                    if (triggeredByAura && (triggeredByAura->GetAuraType() == SPELL_AURA_PROC_TRIGGER_SPELL))
                    {
                        if (SpellInfo const* triggerEntry = sSpellMgr->GetSpellInfo(triggeredByAura->GetSpellInfo()->Effects[triggeredByAura->GetEffIndex()].TriggerSpell))
                        {
                            if (triggerEntry->Dispel == DISPEL_POISON || triggerEntry->Dispel == DISPEL_MAGIC || triggerEntry->Dispel == DISPEL_DISEASE)
                            {
                                missChance += float(target->GetMaxPositiveAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(triggerEntry->Dispel)));
                                missChance += float(target->GetMaxNegativeAuraModifierByMiscValue(SPELL_AURA_MOD_DEBUFF_RESISTANCE, int32(triggerEntry->Dispel)));
                                found = true;
                                break;
                            }
                        }
...
void Unit::ApplyAttackTimePercentMod(WeaponAttackType att, float val, bool apply)
{
    ...
    for (Unit::ControlList::iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
        if ((*itr)->IsPet())
            (*itr)->ToCreature()->ToPet()->UpdateHaste(att, val, apply);
}
...
void Unit::ApplyCastTimePercentMod(float val, bool apply)
{
    ...
    for (Unit::ControlList::iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
        if ((*itr)->IsPet())
            (*itr)->ToCreature()->ToPet()->UpdateHaste(MAX_ATTACK, val, apply);
}
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (victim->ToCreature() && victim->GetOwner() && victim->ToTempSummon())
        if (!victim->IsHunterPet())
            victim->ToTempSummon()->UnSummon();
}
...
bool Unit::IsInAir(float x, float y, float z, float height, bool vmap) const
{
    if (!x)
        x = GetPositionX();
    if (!y)
    ...
    float mapheight = GetMap()->GetHeight(GetPhaseMask(), x, y, z, vmap);

    if (mapheight == VMAP_INVALID_HEIGHT || mapheight == INVALID_HEIGHT || mapheight == VMAP_INVALID_HEIGHT_VALUE)
        return true;

    return (z - mapheight) >= height;
}
...
void Unit::StopAttackFaction(uint32 faction_id)
{
    ...
    for (ControlList::const_iterator itr = m_Posessing.begin(); itr != m_Posessing.end(); ++itr)
        (*itr)->StopAttackFaction(faction_id);
...
void CharmInfo::SaveStayPosition()
{
    ...
    // Finalized means we have no final destination
    if (_unit->movespline->Finalized())
        stayPos = G3D::Vector3(_unit->GetPositionX(), _unit->GetPositionY(), _unit->GetPositionZ());
...
void CharmInfo::SetIsLockedTarget(bool val)
{
    _isLockedTarget = val;
}

bool CharmInfo::IsLockedTarget()
{
    return _isLockedTarget;
}

void CharmInfo::HandleTaunt(bool apply, bool init)
{
    if (apply)
    {
        if (!_unit->ToCreature())
        ...
        if (!_unit->ToCreature()->HasUnitTypeMask(UNIT_MASK_MINION|UNIT_MASK_GUARDIAN|UNIT_MASK_CONTROLABLE_GUARDIAN))
            return;

        if (_unit->ToCreature()->GetReactState() == REACT_PASSIVE)
            return;

        if (!init)
            SetIsLockedTarget(true);
