        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case 29264: // Feral Spirit
                {
                    ...
                    if (GetOwner()->GetAuraEffect(63271, 0)) // Glyph of Feral Spirit
                        dmg_multiplier += 0.3f;

                    SetBonusDamage(int32(GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier));

                    // 14AP == 1dps, wolf's strike speed == 2s so dmg = basedmg + AP / 14 * 2
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float((petlevel * 4 - petlevel) + (GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier * 2 / 14)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float((petlevel * 4 + petlevel) + (GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * dmg_multiplier * 2 / 14)));
...
void Pet::ResetSpellCooldowns(bool resetCategory)
{
    // notify player
    if (GetOwner())
    {
        for (CreatureSpellCooldowns::const_iterator itr2 = m_CreatureSpellCooldowns.begin(); itr2 != m_CreatureSpellCooldowns.end(); ++itr2)
            GetOwner()->SendClearCooldown(itr2->first, this);
    }
