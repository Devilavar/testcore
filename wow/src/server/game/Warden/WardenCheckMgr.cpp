...
void WardenCheckMgr::LoadWardenChecks()
{
    ...
    {
        TC_LOG_INFO("anticheat", ">> Warden disabled, loading checks skipped.");
        return;
    }
...
void WardenCheckMgr::LoadWardenOverrides()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_WARDEN_ENABLED))
    {
        TC_LOG_INFO("anticheat", ">> Warden disabled, loading check overrides skipped.");
        return;
    }
    ...
    do
    {
        ...
        if (action > WARDEN_ACTION_BAN)
            TC_LOG_ERROR("anticheat", "Warden check override action out of range (ID: %u, action: %u)", checkId, action);
        // Check if check actually exists before accessing the CheckStore vector
        else if (checkId > CheckStore.size())
            TC_LOG_ERROR("anticheat", "Warden check action override for non-existing check (ID: %u, action: %u), skipped", checkId, action);
        else
        {
