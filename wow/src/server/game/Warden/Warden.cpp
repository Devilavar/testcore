...
{
    TC_LOG_DEBUG("anticheat", "WARDEN:           Send module to client");
...
{
    TC_LOG_DEBUG("anticheat", "WARDEN:           Request module");
    ...
    if (_initialized)
    {
        ...
        if (_dataSent)
        {
            ...
            if (maxClientResponseDelay > 0)
            {
                ...
                {
                    TC_LOG_WARN("anticheat", "WARDEN:           %s (latency: %u, IP: %s) exceeded Warden module response delay for more than %s - disconnecting client",
...
bool Warden::IsValidCheckSum(uint32 checksum, const uint8* data, const uint16 length)
{
    ...
    {
        TC_LOG_DEBUG("anticheat", "WARDEN:           CHECKSUM IS NOT VALID");
        return false;
    }
    else
    {
        TC_LOG_DEBUG("anticheat", "WARDEN:           CHECKSUM IS VALID");
        return true;
    }
}
...
void WorldSession::HandleWardenDataOpcode(WorldPacket& recvData)
{
    ...
    uint8 opcode;
    recvData >> opcode;
    TC_LOG_DEBUG("anticheat", "WARDEN:           Got packet, opcode %02X, size %u", opcode, uint32(recvData.size()));
    recvData.hexlike();
    ...
    switch (opcode)
    {
        ...
        case WARDEN_CMSG_MEM_CHECKS_RESULT:
            TC_LOG_DEBUG("anticheat", "WARDEN:           NYI WARDEN_CMSG_MEM_CHECKS_RESULT received!");
            break;
        ...
        case WARDEN_CMSG_MODULE_FAILED:
            TC_LOG_DEBUG("anticheat", "WARDEN:           NYI WARDEN_CMSG_MODULE_FAILED received!");
            break;
        default:
            TC_LOG_DEBUG("anticheat", "WARDEN:           Got unknown warden opcode %02X of size %u.", opcode, uint32(recvData.size() - 1));
            break;
    }
}
