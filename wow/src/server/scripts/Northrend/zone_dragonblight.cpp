...
npc_vengeful_geist
*/
enum ResqueTownSquare 
{
    NPC_TRAPPED_VILLAGER                = 27359,
    QUEST_RESCUE_FROM_TOWN_SQUARE       = 12253,
};

class npc_vengeful_geist : public CreatureScript
{
public:

    npc_vengeful_geist() : CreatureScript("npc_vengeful_geist") { }

    struct npc_vengeful_geistAI : public ScriptedAI 
    {

        npc_vengeful_geistAI(Creature * creature) : ScriptedAI(creature) { }
        ...
        void JustDied(Unit * killer) override
        {
            ...
            if (Creature * villager = GetClosestCreatureWithEntry(me, NPC_TRAPPED_VILLAGER, 15.0f)) 
            {
                if (player->GetQuestStatus(QUEST_RESCUE_FROM_TOWN_SQUARE) == QUEST_STATUS_INCOMPLETE) 
                {
                    player->KilledMonsterCredit(NPC_TRAPPED_VILLAGER, 0);
                    villager->AI()->Talk(0, player);
                    villager->HandleEmoteCommand(EMOTE_ONESHOT_CHEER_NO_SHEATHE);
    ...
    {
        return new npc_vengeful_geistAI(creature);
    }
};
