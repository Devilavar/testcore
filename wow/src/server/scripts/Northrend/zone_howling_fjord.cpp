...
public:
    ...
    struct npc_feknut_bunnyAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            CheckTimer = 3000;
            Checked = false;
            me->SetDisplayId(11686); // invisble 
...
/*######
## npc_trained_rock_falcon
######*/

class npc_trained_rock_falcon : public CreatureScript
{
public:
    npc_trained_rock_falcon() : CreatureScript("npc_trained_rock_falcon") { }

    struct npc_trained_rock_falconAI : public ScriptedAI
    {
        npc_trained_rock_falconAI(Creature* creature) : ScriptedAI(creature) { }

        void Reset() override { }
        void EnterCombat(Unit* /*who*/) override { }

        // Initialize the action bar without the melee attack command and
        // put spells in right place
        void initializeSpellBar() 
        {
            me->InitCharmInfo();
            ...
            {
                uint32 spellId = me->ToCreature()->m_spells[i];
                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
                if (spellInfo && !(spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD))
                    if (!spellInfo->IsPassive())
                        me->GetCharmInfo()->AddSpellToActionBar(spellInfo, ACT_PASSIVE);
            }
        }
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_trained_rock_falconAI(creature);
    }
};
...
enum AllianceBanner 
{
    ...
    WINTERSCRON_DESPAWN_TIME    = 20 * IN_MILLISECONDS,
};

Position const SummonDefenderPositions[3] =
{
    {1526.36f, -5305.72f, 198.158f, 3.592f},
    {1508.58f, -5331.28f, 197.336f, 1.488f},
    {1493.72f, -5316.14f, 195.236f, 0.392f},
};
...
public:
    ...
    {
        npc_alliance_bannerAI(Creature* creature) : ScriptedAI(creature) { }
        uint32 waveTimer;
        uint8 killCounter;
        ...
        // prevent banner from entering combat
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (waveTimer < diff) 
            {
                Position pos = SummonDefenderPositions[urand(0, 2)];
                if (Creature* vrykul = me->SummonCreature(NPC_WINTERSKORN_DEFENDER, pos.GetPositionX(), pos.GetPositionY(),
                        pos.GetPositionZ(), pos.GetOrientation(), TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, WINTERSCRON_DESPAWN_TIME)) 
                {
                    vrykul->AI()->AttackStart(me);
                    vrykul->GetMotionMaster()->Clear();
                    vrykul->GetMotionMaster()->MoveChase(me);
                    vrykul->SetSpeed(MOVE_RUN, 1.0f);
                }
        ...
        void SummonedCreatureDies(Creature* summoned, Unit* /*killer*/) override
        {
            if (summoned->GetEntry() == NPC_WINTERSKORN_DEFENDER)
                killCounter++;

            if (killCounter >= 3) {
                Unit* owner = me->GetCharmerOrOwnerOrSelf();
                if (owner && owner->GetTypeId() == TYPEID_PLAYER)
                    owner->ToPlayer()->GroupEventHappens(QUEST_DROP_IT_ROCK_IT, me);
