...
public:
        ...
        void OnCreatureCreate(Creature* creature)
        {
            switch (creature->GetEntry())
            {
                ...
                case NPC_HALION_CONTROLLER:
                    ...
                    break;
                case NPC_ORB_CARRIER: // we use this as a dummy npc basically
                    OrbCarrierGUID = creature->GetGUID();
                    if (GetBossState(DATA_GENERAL_ZARITHRIAN) == DONE && !(GetBossState(DATA_HALION) == DONE)) // this is only called when the creature is created, and since it is a perm. spawn, its only called when instance is created
                    {
        ...
        uint64 GetData64(uint32 type) const override
        {
            switch (type)
            {
                ...
                case DATA_ZARITHRIAN_SPAWN_STALKER_2:
                    return ZarithianSpawnStalkerGUID[type - DATA_ZARITHRIAN_SPAWN_STALKER_1];
                case DATA_HALION:
                    return HalionGUID;
    ...
    protected:
        ...
        uint64 ZarithianSpawnStalkerGUID[4];
