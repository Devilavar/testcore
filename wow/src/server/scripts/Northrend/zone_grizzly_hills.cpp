...
enum MaidenOFAshwoodLake
{
   ...
   QUEST_ITEM_ASHWOOD_BRAND    = 44981
};
...
public:
   ...
   {
      // check if the player doesn't already have the quest item 
      if(!player->HasItemCount(QUEST_ITEM_ASHWOOD_BRAND, 1, true))
      {
         // show gossip when player doesn't have the quest item
   ...
   bool OnGossipSelect(Player* player, Creature* creature /*pCreature*/, uint32 /*sender*/, uint32 action) override
   {
      player->PlayerTalkClass->ClearMenus();
      switch(action)
      {
      case GOSSIP_ACTION_INFO_DEF+1:
         player->CastSpell(player, 62554, true);
