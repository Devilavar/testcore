    ...
    public:
        ...
        struct boss_blood_queen_lana_thelAI : public BossAI
        {
            ...
            void CleanSpells()
            {
                ...
                // unlearn vampiric bite from players
            ...
            Player* SelectRandomTarget(bool includeOfftank, std::list<Player*>* targetList = NULL)
            {
                ...
                for (std::list<HostileReference*>::const_iterator itr = threatlist.begin(); itr != threatlist.end(); ++itr)
                    if (Unit* refTarget = (*itr)->getTarget())
                        if (refTarget != me->GetVictim() && refTarget->GetTypeId() == TYPEID_PLAYER && me->IsValidAttackTarget(refTarget) && (includeOfftank || (refTarget->GetGUID() != _offtankGUID)))
