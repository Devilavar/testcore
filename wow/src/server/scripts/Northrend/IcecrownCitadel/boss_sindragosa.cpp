            ...
            void MovementInform(uint32 type, uint32 point) override
            {
                ...
                switch (point)
                {
                    case POINT_FROSTWYRM_LAND:
                        ...
                        break;
                    case POINT_TAKEOFF:
                        events.ScheduleEvent(EVENT_AIR_MOVEMENT, 1000);
                        break;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_AIR_MOVEMENT:
                            me->GetMotionMaster()->MovePoint(POINT_AIR_PHASE, SindragosaAirPos, false);
                            break;
                        case EVENT_AIR_MOVEMENT_FAR:
                            me->GetMotionMaster()->MovePoint(POINT_AIR_PHASE_FAR, SindragosaAirPosFar, false);
                            break;
                        case EVENT_ICE_TOMB:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, FrostBeaconSelector(me)))
                            {
                        ...
                        case EVENT_LAND:
                        {
                            ...
                            me->GetMotionMaster()->MovePoint(POINT_LAND, SindragosaFlyInPos, false);
                            break;
                        }
