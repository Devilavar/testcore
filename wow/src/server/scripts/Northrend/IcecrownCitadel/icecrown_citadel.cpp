...
{
    { 4130.71f, 2484.10f, 211.033f, 0 }, // 0
    { 4137.93f, 2505.52f, 211.033f, 0 }, // 1
    { 4160.64f, 2528.13f, 211.033f, 0 }, // 2
    { 4180.81f, 2533.88f, 211.033f, 0 }, // 3
    { 4200.92f, 2527.18f, 211.033f, 0 }, // 4
    { 4222.23f, 2503.56f, 211.033f, 0 }, // 5
    { 4229.40f, 2484.63f, 211.033f, 0 }, // 6
    { 4222.01f, 2464.93f, 211.033f, 0 }, // 7
    { 4201.55f, 2441.03f, 211.033f, 0 }, // 8
    { 4181.29f, 2433.38f, 211.033f, 0 }, // 9
    { 4161.86f, 2441.94f, 211.033f, 0 }, // 10
    { 4138.78f, 2463.95f, 211.033f, 0 }, // 11
    { 4181.45f, 2417.89f, 211.033f, 0 }, // east middle 12
    { 4186.45f, 2417.89f, 211.033f, 0 }, // east right 13
    { 4176.45f, 2417.89f, 211.033f, 0 }, // east left 14
    { 4181.45f, 2553.43f, 211.033f, 0 }, // west middle 15
    { 4186.45f, 2553.43f, 211.033f, 0 }, // west left 16
    { 4176.45f, 2553.43f, 211.033f, 0 }, // west right 17
};
...
enum LightsHammerCreatures
{
    NPC_EBON_COMMANDER   = 37967,
    NPC_EBON_CHAMPION    = 37996,
    NPC_ARGENT_COMMANDER = 37965,
    NPC_ARGENY_CHAMPION  = 37928,
    NPC_FURY             = 37586,
};

class npc_generic_lights_hammer : public CreatureScript
{
public:
    npc_generic_lights_hammer() : CreatureScript("npc_generic_lights_hammer") { }

    struct npc_generic_lights_hammerAI : public CreatureAI
    {
        npc_generic_lights_hammerAI(Creature* creature) : CreatureAI(creature)
        {
            me->SetReactState(REACT_DEFENSIVE);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            switch (me->GetEntry())
            {
                case NPC_EBON_COMMANDER:
                    break;
                default:
                    if (me->GetDistance(-76, 2214, 28) >= 75.0f && me->AI())
    ...
    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_generic_lights_hammerAI(creature);
    }
};
...
enum SpellzATStart
{
SPELL_THIRTYPERCENT_ONE = 0,
SPELL_THIRTYPERCENT_TWO = 0,
};
...
public:
    ...
    bool OnGossipHello(Player *_player, Creature *_creature) override
    {
        if (_player->HasAura(SPELL_THIRTYPERCENT_ONE))
        {
        ...
        {
            _creature->MonsterWhisper("You already asked me to make it more challengeable!", _player);
            return true;
        }
    }

    bool OnGossipSelect(Player *_player, Creature *_creature, uint32 sender, uint32 uiAction) override
    {
        if (sender == GOSSIP_SENDER_MAIN)
        {
            ...
            switch(uiAction)
            {
                case 1:
                    ...
                    _creature->MonsterWhisper("Hope you can make it without my power!", _player, true);
                    _player->RemoveAura(SPELL_THIRTYPERCENT_ONE);
                    break;
                default:
                    break;                   
            }
...
/*
class npc_muradin_challenge : public CreatureScript
{
public:
    npc_muradin_challenge() : CreatureScript("npc_muradin_challenge") { }
 
    bool OnGossipHello(Player *_player, Creature *_creature) override
    {
        if (_player->HasAura(SPELL_THIRTYPERCENT_TWO))
        {
            _player->ADD_GOSSIP_ITEM(3,"Muradin, i want to make it more challengeable!"                       , GOSSIP_SENDER_MAIN, 1);
            _player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, _creature->GetGUID());
            return true;
        }
        else
    ...
    bool OnGossipSelect(Player *_player, Creature *_creature, uint32 sender, uint32 uiAction) override
    {
        if (sender == GOSSIP_SENDER_MAIN)
        {
            ...
            switch(uiAction)
            {
                case 1:
                    _player->CLOSE_GOSSIP_MENU();
                    _creature->MonsterWhisper("Hope you can make it without my power!", _player);
            ...
            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
            {
                if (Player* player = i->GetSource())
                {
                    if (me->GetDistance2d(player) < 80.0f)
                    {
                        ...
                        Position HomePos = me->GetPosition();
