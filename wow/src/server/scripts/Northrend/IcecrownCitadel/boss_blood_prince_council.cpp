...
class boss_prince_keleseth_icc : public CreatureScript
{
    public:
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    ...
                    case ACTION_REMOVE_INVOCATION:
                        ...
                        _isEmpowered = false;
                        break;
                    case ACTION_SUMMON_DARK_NUCLEUS:
                        CrimsonHall.m_positionX = 4657.922363f;
                        CrimsonHall.m_positionY = 2769.246338f;
                        CrimsonHall.m_positionZ = 361.026581f;
                        me->GetRandomPoint(CrimsonHall, 20.0f, x, y, z);
                        _groundZ = me->GetMap()->GetHeight(x, y, z+20.0f, true, 500.0f);
                        me->SummonCreature(NPC_DARK_NUCLEUS, x, y, _groundZ, me->GetOrientation(), TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000);
                        break;
                    default:
                        break;
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SHADOW_RESONANCE:
                            ...
                            DoAction(ACTION_SUMMON_DARK_NUCLEUS);
                            events.ScheduleEvent(EVENT_SHADOW_RESONANCE, urand(10000, 15000));
                            break;
        ...
        private:
            uint32 _spawnHealth;
            bool _isEmpowered;
            Position CrimsonHall;
            float x, y, z, _groundZ;
        };
    ...
    public:
        ...
        struct boss_prince_valanarAI : public BossAI
        {
            ...
            void EnterCombat(Unit* /*who*/) override
            {
                ...
                if (Creature* keleseth = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_PRINCE_KELESETH_GUID)))
                {
                    keleseth->AI()->AttackStart(me->GetVictim());
                }

                if (Creature* taldaram = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_PRINCE_TALDARAM_GUID)))
                {
                    taldaram->AI()->AttackStart(me->GetVictim());
                }
    ...
    public:
        ...
        struct npc_kinetic_bombAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                me->SetReactState(REACT_PASSIVE);
                _speed = IsHeroic() ? 0.3f : 0.15f;
                _jumpSpeed = _speed * 10.0f;
                me->SetSpeed(MOVE_FLIGHT, _speed, true);
            ...
            void DoAction(int32 action) override
            {
                ...
                switch (action)
                {
                    ...
                    case ACTION_KINETIC_BOMB_JUMP:
                        ...
                        if (!_startZ) // not initialized
                            _startZ = posZ;

                        _diffZ += IsHeroic() ? 4.0f : 2.0f;
                        if (_startZ+_diffZ > _groundZ+30.0f) // height cap of 30
                            _diffZ = _groundZ+30.0f - _startZ;

                        _distZ = (_startZ+_diffZ) - posZ; // how far to reach destination
                        if (_distZ < 0.0f)
                            _distZ = 0.0f;

                        _events.CancelEvent(EVENT_CONTINUE_FALLING);
                        me->GetMotionMaster()->Clear();
                        me->GetMotionMaster()->MoveJump(_x, _y, _startZ+_diffZ, _jumpSpeed, _jumpSpeed); // SpeedZ is broken
                        _events.ScheduleEvent(EVENT_CONTINUE_FALLING, uint32((_distZ*1000.0f)/_jumpSpeed)); // hack due to MotionMaster being partially broken
                        break;
                    default:
                        break;
                }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if ((me->GetPositionZ() - _groundZ) <= 2.0f && me->HasAura(SPELL_KINETIC_BOMB_VISUAL))
                {
    ...
    public:
        ...
        struct npc_dark_nucleusAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                me->SetReactState(REACT_DEFENSIVE);
                me->SetVisible(false);
                me->SetDisplayId(1126); // trigger display
                active = false;
                 me->GetPosition(x, y, _groundZ);
                _groundZ = me->GetMap()->GetHeight(x, y, _groundZ, true, 500.0f);

                if (me->GetPositionZ() == -200000.0f) // spawned at invalid point
                    me->DespawnOrUnsummon();

                _events.ScheduleEvent(EVENT_ACTIVATE, 2000); // allow 2 sec to get into the proper position
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_ACTIVATE:
                            me->SetDisplayId(DISPLAY_DARK_NUCLEUS);
                            me->SetVisible(true);
                ...
                if (!active)
                {
                    // HACK: Correct position
                    Movement::MoveSplineInit init(me);
                    init.MoveTo(me->GetPositionX(), me->GetPositionY(), _groundZ+me->GetFloatValue(UNIT_FIELD_HOVERHEIGHT));
                    init.SetFacing(me->GetOrientation());
                    init.Launch();
                    me->SetHomePosition(me->GetPositionX(), me->GetPositionY(), _groundZ+me->GetFloatValue(UNIT_FIELD_HOVERHEIGHT), me->GetOrientation());
                }
    ...
    public:
        ...
        struct npc_shock_vortexAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                _events.ScheduleEvent(EVENT_ACTIVATE, 10000); // 10s to activate
            }
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetIcecrownCitadelAI<npc_shock_vortexAI>(creature);
        }
};
