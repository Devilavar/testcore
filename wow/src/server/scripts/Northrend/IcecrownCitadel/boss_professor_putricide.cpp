...
Position const FloodCorners[] = 
{
    {4476.401f, 3106.296f, 366.0f}, // NE
    {4476.833f, 3167.451f, 366.0f}, // NW
    {4414.959f, 3106.476f, 366.0f}, // SE
    {4414.979f, 3167.879f, 366.0f}, // SW
};

// used in Rotface encounter
#define OOZE_FLOOD RAID_MODE<int32>(69782, 69796, 69798, 69801)
...
class boss_professor_putricide : public CreatureScript
{
    public:
                ...
                switch (action)
                {
                    ...
                    case ACTION_ROTFACE_COMBAT:
                    {
                        ...
                        if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE)))
                        {
                            ...
                            GetCreatureListWithEntryInGrid(list, rotface, NPC_PUDDLE_STALKER, 100.0f);
                            ...
                            for (std::list<Creature*>::iterator itr = list.begin(); itr != list.end(); ++itr)
                            {
                                ...
                                for (uint8 i = 0; i < 4; ++i)
                                {
                                    dist[i] = (*itr)->GetDistance2d(FloodCorners[i].m_positionX, FloodCorners[i].m_positionY);
                                    if (dist[i] < dist[closest])
                                        closest = i;
                                }

                                _oozeFloods.push_back(std::make_pair(closest, (*itr)));
                            }
                        }
                        break;
                    }
                    case ACTION_ROTFACE_OOZE:
                    {
                        ...
                        uint8 corner = urand(0, 3);
                        for (OozeFloods::iterator itr = _oozeFloods.begin(); itr != _oozeFloods.end(); ++itr)
                        {
                            if (itr->first != corner)
                                continue;

                            if (Creature* trigger = itr->second)
                                trigger->CastSpell(trigger, OOZE_FLOOD, true, NULL, NULL, me->GetGUID());
                        }
                        break;
                    }
        ...
        private:
            ...
            typedef std::list<std::pair<uint8, Creature*>> OozeFloods;
            OozeFloods _oozeFloods;
