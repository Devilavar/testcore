...
enum Events
{
    ...
    // when players leave frostmourne
    EVENT_FROSTMOURNE_EXIT          = 64,
    EVENT_RESTORED_SOUL             = 65,

    // Spirit Bombs explosion delay
    EVENT_SPELL_EXPLOSION           = 66,
};
...
class boss_the_lich_king : public CreatureScript
{
    public:
        ...
        struct boss_the_lich_kingAI : public BossAI
        {
            ...
            void CleanSpells()
            {
                instance->DoRemoveAurasDueToSpellOnPlayers(70867); // Essence of the Blood Queen
                instance->DoRemoveAurasDueToSpellOnPlayers(71473); // Essence of the Blood Queen
                instance->DoRemoveAurasDueToSpellOnPlayers(71532); // Essence of the Blood Queen
                instance->DoRemoveAurasDueToSpellOnPlayers(71533); // Essence of the Blood Queen
                instance->DoRemoveAurasDueToSpellOnPlayers(70879); // Essence of the Blood Queen PLR
                instance->DoRemoveAurasDueToSpellOnPlayers(71525); // Essence of the Blood Queen PLR
                instance->DoRemoveAurasDueToSpellOnPlayers(71530); // Essence of the Blood Queen PLR
                instance->DoRemoveAurasDueToSpellOnPlayers(71531); // Essence of the Blood Queen PLR
                instance->DoRemoveAurasDueToSpellOnPlayers(70923); // Unconrollable Frenzy
                        ...
                        if (!PlayerList.isEmpty())
                            for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                                if (Player* player = i->GetSource())
                                    if (player->HasSpell(71726))
                                        player->removeSpell(71726);
                    }
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_HARVEST_SOULS:
                            ...
                            // must deactivate the vile spirits when LK casts harvest soul
                            triggers.clear();
                            GetCreatureListWithEntryInGrid(triggers, me, NPC_VILE_SPIRIT, 100.0f);
                            if (!triggers.empty())
                            {
                                triggers.sort(Trinity::ObjectDistanceOrderPred(me, true));
                                for (std::list<Creature*>::iterator itr = triggers.begin(); itr != triggers.end(); ++itr)
                                {
                                    (*itr)->m_Events.KillAllEvents(true);
                                    (*itr)->m_Events.AddEvent(new VileSpiritActivateEvent((*itr)), (*itr)->m_Events.CalculateTime(50000));
                                    (*itr)->GetMotionMaster()->MoveRandom(10.0f);
                                    (*itr)->SetReactState(REACT_PASSIVE);
                                }
                            }
                            triggers.clear();
                            break;
                        case EVENT_FROSTMOURNE_EXIT:
                            instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_HARVEST_SOULS_TELEPORT); // Removes the DoT that players get inside Frostmourne
                            break;
                        case EVENT_FROSTMOURNE_HEROIC:
                            ...
                            events.ScheduleEvent(EVENT_FROSTMOURNE_EXIT, 39500);
                            break;
            ...
            void MovementInform(uint32 type, uint32 id) override
            {
                ...
                switch (id)
                {
                    case POINT_DROP_PLAYER:
                        // Workaround for movement generator issue
                        // If valk'yrs movement stopped (i.e root, stun, etc), it would assume it has reached drop_point
                        _current = me->GetPosition();
...
class npc_terenas_menethil : public CreatureScript
{
    public:
        ...
        struct npc_terenas_menethilAI : public ScriptedAI
        {
            ...
            bool CanAIAttack(Unit const* target) const override
            {
                return target->GetEntry() != NPC_THE_LICH_KING && !IsHeroic();
            }
                    ...
                    switch (eventId)
                    {
                        case EVENT_FROSTMOURNE_TALK_1:
                            ...
                            if (IsHeroic())
                            {
                                ...
                                _events.ScheduleEvent(EVENT_RESTORED_SOUL, 39500);
                            }
                            break;
                        ...
                        case EVENT_TELEPORT_BACK:
                            ...
                            break;
                        case EVENT_RESTORED_SOUL:
                            triggers.clear();
                            GetCreatureListWithEntryInGrid(triggers, me, NPC_WORLD_TRIGGER_INFINITE_AOI, 100.0f);
                            if (!triggers.empty())
                            {
                                triggers.sort(Trinity::ObjectDistanceOrderPred(me, true));
                                Creature* spawner = triggers.front();
                                spawner->RemoveAura(SPELL_SUMMON_SPIRIT_BOMB_1);
                                spawner->RemoveAura(SPELL_SUMMON_SPIRIT_BOMB_2);
                                spawner->m_Events.KillAllEvents(true);
                            }
                            triggers.clear();
                            GetCreatureListWithEntryInGrid(triggers, me, NPC_SPIRIT_BOMB, 100.0f);
                            if (!triggers.empty())
                            {
                                triggers.sort(Trinity::ObjectDistanceOrderPred(me, true));
                                for (std::list<Creature*>::iterator itr = triggers.begin(); itr != triggers.end(); ++itr)
                            ...
                            triggers.clear();
                            GetCreatureListWithEntryInGrid(triggers, me, NPC_WICKED_SPIRIT, 100.0f);
                            if (!triggers.empty())
                            {
                                triggers.sort(Trinity::ObjectDistanceOrderPred(me, true));
                                for (std::list<Creature*>::iterator itr = triggers.begin(); itr != triggers.end(); ++itr)
    ...
    public:
        ...
        struct npc_spirit_bombAI : public CreatureAI
        {
            ...
            void IsSummonedBy(Unit* /*summoner*/) override
            {
                ...
                me->GetMotionMaster()->MovePoint(POINT_GROUND, destX, destY, destZ, false);
            }
...
class spell_the_lich_king_valkyr_target_search : public SpellScriptLoader
{
    public:
        ...
        class spell_the_lich_king_valkyr_target_search_SpellScript : public SpellScript
        {
            ...
            {
                PreventHitDefaultEffect(effIndex);
                if (GetCaster()->GetTypeId() == TYPEID_UNIT && _target)
                    GetCaster()->GetMotionMaster()->MoveCharge(_target->GetPositionX(), _target->GetPositionY(), GetCaster()->GetPositionZ(), SPEED_CHARGE, POINT_CHARGE);
            }
    ...
    public:
        ...
        class spell_the_lich_king_vile_spirit_move_target_search_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
                ...
                GetCaster()->SetSpeed(MOVE_FLIGHT, 0.5f);
            }
    ...
    public:
        ...
        class spell_the_lich_king_vile_spirit_damage_target_search_SpellScript : public SpellScript
        {
            ...
            void CheckTargetCount(std::list<WorldObject*>& targets)
            {
                ...
                if (GetCaster()->ToCreature()->GetEntry() != NPC_WICKED_SPIRIT) // these should never not be selectable
                    GetCaster()->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
            }
