...
#define FRIENDS_COUNT                            RAID_MODE<uint8>(4, 8) // @todo this needs script to check how much friendly npcs are alive
...
Position const SummonPositions[8] =
{
    ...
    { 1994.22f, -254.44f, 432.767f, 1.24f }, // Eivi Nightfeather    &&  Tor Greycloud
        ...
        struct boss_hodirAI : public BossAI
        {
            ...
            void DamageTaken(Unit* /*who*/, uint32& damage) override
            {
                if (damage >= me->GetHealth())
                {
					...
					if (Is25ManRaid()) // 25n/25hc
					{
						instance->CompleteAchievement(2962); // 25n
						instance->CompleteAchievement(2968); // 25n
						instance->CompleteAchievement(3184); // 25n
						instance->CompleteAchievement(2965); // 25n
					}
					else // 10n/10hc
					{
						instance->CompleteAchievement(2961); // 10n
						instance->CompleteAchievement(2967); // 10n 
						instance->CompleteAchievement(3182); // 10n
						instance->CompleteAchievement(2963); // 10n
					}
