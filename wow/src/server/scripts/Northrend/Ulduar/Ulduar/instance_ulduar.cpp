...
#define ACHIEVEMENT_I_CHOOSE_YOU_RUNEMASTER		RAID_MODE(2939, 2942);
#define ACHIEVEMENT_I_CHOOSE_YOU_STEELBREAKER	RAID_MODE(2941, 2944);
#define ACHIEVEMENT_BUT_IM_ON_YOUR_SIDE			RAID_MODE(2945, 2946);
...
class instance_ulduar : public InstanceMapScript
{
    public:
        ...
        public:
            ...
            void OnCreatureDeath(Creature* creature, Player* player)
            {
				// hacky part starts
				// achievements for the 3 iron bosses
				/*if (InstanceScript* instance = creature->GetInstanceScript())
					{
						Creature* Brundir = ObjectAccessor::GetCreature(*creature, instance->GetData64(DATA_BRUNDIR));
						Creature* Molgeim = ObjectAccessor::GetCreature(*creature, instance->GetData64(DATA_MOLGEIM));
						Creature* SteelBreaker = ObjectAccessor::GetCreature(*creature, instance->GetData64(DATA_STEELBREAKER));

						if (Brundir->HasUnitState(UNIT_STATE_DIED) && SteelBreaker->HasUnitState(UNIT_STATE_DIED)) // |   If both bosses are dead >
							instance->CompleteAchievement(ACHIEVEMENT_I_CHOOSE_YOU_RUNEMASTER);                    // |   Reward Achievement: I Choose You RuneMaster

						if (Brundir->HasUnitState(UNIT_STATE_DIED) && Molgeim->HasUnitState(UNIT_STATE_DIED)) // |   If both bosses are dead >
							instance->CompleteAchievement(ACHIEVEMENT_I_CHOOSE_YOU_STEELBREAKER);             // |   Reward Achievement: I Choose You Steelbreaker

						if (Brundir->HasUnitState(UNIT_STATE_DIED) && Brundir->HasUnitState(UNIT_STATE_DIED) && Brundir->HasUnitState(UNIT_STATE_DIED)) // make sure all are dead
								if (player->HasAura(0)) // iron boot flask
									instance->CompleteAchievement(ACHIEVEMENT_BUT_IM_ON_YOUR_SIDE);
					}
					*/
				// hacky parts end
            ...
            void OnUnitDeath(Unit* unit)
            {
                if (Player* player = unit->ToPlayer())
                    ...
                    OnCreatureDeath(creature, player);
            }
