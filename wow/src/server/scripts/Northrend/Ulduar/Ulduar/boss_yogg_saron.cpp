        ...
        struct npc_yogg_saron_encounter_controllerAI : public BossAI
        {
                ...
                switch (action)
                {
                    case ACTION_YOGGSARON_KILLED:
                        ...
                        if (events.GetNextEventTime(EVENT_ENRAGE) >= 8*MINUTE*IN_MILLISECONDS)
                            instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_HE_S_NOT_GETTING_ANY_OLDER_10, ACHIEVMENT_HE_S_NOT_GETTING_ANY_OLDER_25));
                        if (!usedMindcontrol)
                            instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_DRIVE_ME_CRAZY_10, ACHIEVMENT_DRIVE_ME_CRAZY_25));
                        if (GetDifficulty() == RAID_DIFFICULTY_25MAN_NORMAL && !CountKeepersActive())
                            instance->CompleteAchievement(ACHIEVMENT_REALM_FIRST_DEATHS_DEMISE);
                        switch (CountKeepersActive())
                        {
                            case 0:
                                instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_ALONE_IN_THE_DARKNESS_10, ACHIEVMENT_ALONE_IN_THE_DARKNESS_25));
                            case 1:
                                instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_ONE_LIGHTS_IN_THE_DARKNESS_10, ACHIEVMENT_ONE_LIGHTS_IN_THE_DARKNESS_25));
                            case 2:
                                instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_TWO_LIGHTS_IN_THE_DARKNESS_10, ACHIEVMENT_TWO_LIGHTS_IN_THE_DARKNESS_25));
                            case 3:
                                instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_THREE_LIGHTS_IN_THE_DARKNESS_10, ACHIEVMENT_THREE_LIGHTS_IN_THE_DARKNESS_25));
                                break;
                            default: 
                                break;
                        }
            ...
            void DoMadness()
            {
				...
				//{
						...
						// custom hack for 3rd phase, (we skip 2nd because portals does not work
						if (Creature* yogg = ObjectAccessor::GetCreature(*me, guidYogg))
							UpdatePhase(PHASE_YOGG);
				//}
                ...
                if (myPhase >= PHASE_BRAIN)
                {
                    if (emote == TEXT_EMOTE_KISS)
                    {
						...
						if (me->IsWithinLOS(pPlayer->GetPositionX(), pPlayer->GetPositionY(), pPlayer->GetPositionZ()) && me->IsWithinDistInMap(pPlayer, 30.0f))
							instance->CompleteAchievement(RAID_MODE(ACHIEVMENT_KISS_AND_MAKE_UP_10, ACHIEVMENT_KISS_AND_MAKE_UP_25));
                    }
                }
            }
                    ...
                    case ACTION_DO_CHANGE_PHASE:
                        switch (val)
                        {
                            ...
                            case PHASE_YOGG:
								...
								// since the gaze is causing client crash, let's make it a bit custom.. add tentacles from phase 2 in bigger respawn time >>
								events.ScheduleEvent(EVENT_TENTACLE, urand(15 * IN_MILLISECONDS, 15 * IN_MILLISECONDS), ID_GROUP_NON_SHATTERED, PHASE_YOGG);
								events.ScheduleEvent(EVENT_TENTACLE_1, 7 * IN_MILLISECONDS, ID_GROUP_NON_SHATTERED, PHASE_YOGG);
                                break;
                            default:
                                break;
                        }
