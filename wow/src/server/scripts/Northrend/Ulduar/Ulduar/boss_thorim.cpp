    ...
    public:
        ...
        struct boss_thorimAI : public BossAI
        {
            ...
            void EncounterPostProgress()
            {
                ...
                if (HardMode)
                {
                    instance->CompleteAchievement(ACHIEVEMENT_LOSE_ILLUSION);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_BERSERK_PHASE_1:
							// crashing client because it applies on the entrie instance. no condition?
							// todo later.
                        ...
                        case EVENT_BERSERK_PHASE_2:
							me->AddAura(SPELL_BERSERK_PHASE_2, me);
                            Talk(SAY_BERSERK);
                            break;
                        default:
                            break;
            ...
            void JustSummoned(Creature* summon)
            {
				...
				// mhm.. oneshotting players, probably not buggy..
                ...
                if (phase == PHASE_ARENA_ADDS && attacker && instance)
                {
                    ...
                    if (colossus && colossus->isDead() && giant && giant->isDead() && me->IsWithinDistInMap(attacker, 50.0f) && attacker->ToPlayer())
                    {
                        ...
                        if (EncounterTime <= MAX_HARD_MODE_TIME)
                        {
                            ...
                            // Achievement Siffed
                            if (instance)
                                instance->CompleteAchievement(ACHIEVEMENT_SIFFED);
                        }
                        else
...
class go_thorim_lever : public GameObjectScript
{
    public:
        ...
        bool OnGossipHello(Player* /*pPlayer*/, GameObject* pGo)
        {
            ...
            if (GameObject* door = pGo->FindNearestGameObject(GO_DARK_IRON_PORTCULLIS, 30.0f))
            {
            ...
            return true;
			TC_LOG_INFO("server.worldserver", "5");
        }
};
