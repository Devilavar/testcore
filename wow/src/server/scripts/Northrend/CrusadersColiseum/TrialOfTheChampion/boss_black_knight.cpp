            ...
            if (Phase == INTRO)
            {
                if (uiIntroTimer <= uiDiff)
                {
                    switch (uiIntroPhase)
                    {
                        ...
                        case 2:
                        {
                            if (Creature* pAnnouncer = Unit::GetCreature(*me, instance->GetData64(DATA_ANNOUNCER)))
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (instance)
            {
                instance->SetData(BOSS_BLACK_KNIGHT, DONE);
                 instance->CompleteAchievement(4297);
                 instance->CompleteAchievement(4298);
                 instance->CompleteAchievement(3778);
                 instance->CompleteAchievement(4296);
                if (GameObject* go = GameObject::GetGameObject(*me, instance->GetData64(DATA_MAIN_GATE1)))
                    instance->HandleGameObject(go->GetGUID(), true);
            }
        }
