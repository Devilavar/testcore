...
public:
    ...
    struct boss_lich_king_horAI : public npc_escortAI
    {
        ...
        void WaypointReached(uint32 i) override
        {
            ...
            switch(i)
            {
                case 66:
                    ...
                    if (Creature* escapeleader = ((Creature*)Unit::GetUnit((*me), pInstance->GetData64(DATA_ESCAPE_LEADER))))
                        me->CastSpell(escapeleader, SPELL_HARVEST_SOUL, false);
                    me->setActive(false);
                    if (GameObject* gunship = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_GUNSHIP)))
                        gunship->SetPhaseMask(1, true);

                    if (Creature* captain = pInstance->instance->GetCreature(pInstance->GetData64(DATA_CAPTAINS)))
                    {
                        captain->SetPhaseMask(1, true);
                        captain->SummonCreature(NPC_WARSONG_CANNON, 5211.55f, 1582.87f, 793.857f, 1.35741f, TEMPSUMMON_MANUAL_DESPAWN, 0);
                        captain->SummonCreature(NPC_WARSONG_CANNON, 5224.60f, 1579.75f, 794.432f, 1.35741f, TEMPSUMMON_MANUAL_DESPAWN, 0);
                        captain->SummonCreature(NPC_WARSONG_CANNON, 5199.05f, 1586.30f, 794.432f, 1.35741f, TEMPSUMMON_MANUAL_DESPAWN, 0);
                        captain->SummonCreature(NPC_WARSONG_CANNON, 5197.13f, 1519.70f, 793.855f, 4.48017f, TEMPSUMMON_MANUAL_DESPAWN, 0);
                        captain->SummonCreature(NPC_WARSONG_CANNON, 5210.27f, 1517.34f, 794.441f, 4.48017f, TEMPSUMMON_MANUAL_DESPAWN, 0);
                        captain->SummonCreature(NPC_WARSONG_CANNON, 5184.53f, 1523.01f, 793.855f, 4.48017f, TEMPSUMMON_MANUAL_DESPAWN, 0);
                    }
                    break;
                default:
                    break;
        ...
        void JustSummoned(Creature* summoned) override
        {
            ...
            if (Unit* escapeleader = Unit::GetUnit((*me), pInstance->GetData64(DATA_ESCAPE_LEADER)))
            {
                summoned->GetMotionMaster()->MoveChase(escapeleader);
                summoned->AddThreat(escapeleader, 100.0f);
            }
        }
        ...
        void UpdateEscortAI(const uint32 diff) override
        {
            ...
            if (Creature* escapeleader = ((Creature*)Unit::GetUnit((*me), pInstance->GetData64(DATA_ESCAPE_LEADER))))
            {
                if (!WipedGroup && escapeleader->IsWithinDistInMap(me, 2.0f) && pInstance->GetData(DATA_LICHKING_EVENT) == IN_PROGRESS)
                {
                    WipedGroup = true;
                    me->setActive(false);
                    SetEscortPaused(true);
                    ...
                    me->DealDamage(escapeleader, escapeleader->GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false); // Probably a hack
                }
            }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if(pInstance->GetData(DATA_LICHKING_EVENT) == IN_PROGRESS)
            {
                if(Emerge != true)
                {
                    ...
                    {
                        Emerge = true;
                        uiEscapeLeader = pInstance->GetData64(DATA_ESCAPE_LEADER);
                        if(Creature* escapeleader = ((Creature*)Unit::GetUnit((*me), uiEscapeLeader)))
                        {
                            DoResetThreat();
                            me->AI()->AttackStart(escapeleader);
                            me->GetMotionMaster()->Clear();
                            me->GetMotionMaster()->MoveChase(escapeleader);
                        }
                    }
                    else
                        EmergeTimer -= diff;
