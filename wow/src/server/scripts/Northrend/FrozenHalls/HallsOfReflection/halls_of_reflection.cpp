...
const Position MoveDoorPos           = {5264.694336f, 1959.382324f, 707.695007f, 0.808736f};
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            // Remaining Intro Events common for both faction
                case EVENT_INTRO_LK_1:
                    ...
                    if (Creature* uther = me->GetCreature(*me, utherGUID))
                    {
                        if (Creature* lichking = me->GetCreature(*me, lichkingGUID))
                            uther->SetOrientation(0.851610f);
                ...
                case EVENT_INTRO_LK_3:
                     ...
                     if (Creature* uther = me->GetCreature(*me, utherGUID))
                         uther->CastSpell(uther, SPELL_UTHER_DESPAWN, true); // todo, either this spell is broken or it's triggered by another spell, because the LK should be able to cast it on uther.  Currently it cannot be cast on someone else.
                ...
                case EVENT_INTRO_LK_5:
                    ...
                    if (Creature* falric = me->GetCreature(*me, instance->GetData64(DATA_FALRIC)))
                    {
                        ...
                        falric->SetUnitMovementFlags(MOVEMENTFLAG_WALKING);
                    ...
                    if (Creature* marwyn = me->GetCreature(*me, instance->GetData64(DATA_MARWYN)))
                    {
                        ...
                        marwyn->SetUnitMovementFlags(MOVEMENTFLAG_WALKING);
...
public:
    ...
    bool OnGossipSelect(Player* plr, Creature* creature, uint32 /*sender*/, uint32 action) override
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                ...
                CAST_AI(npc_jaina_or_sylvanas_escape_horAI, creature->AI())->Start(false,true);
    ...
    struct npc_jaina_or_sylvanas_escape_horAI : public npc_escortAI
    {
        npc_jaina_or_sylvanas_escape_horAI(Creature* creature) : npc_escortAI(creature)
        {
        ...
        void WaypointReached(uint32 i) override
        {
            switch(i)
            {
                case 3:
                    ...
                    if(GameObject* gate = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_1)))
                    {
                        gate->SetGoState(GO_STATE_READY);
                        m_uiIceWallGUID = gate->GetGUID();
                    }
                    break;
                case 4:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_WALL_01 : SAY_SYLVANA_WALL_01);
                    CastTimer = 1000;
                    HoldTimer = 30000;
                    ...
                    {
                        m_uipWallTargetGUID = wallTarget->GetGUID();
                ...
                case 6:
                    ...
                    if (Creature* wallTarget = m_pInstance->instance->GetCreature(m_uipWallTargetGUID))
                    {
                    ...
                    break;
                case 8:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_WALL_02 : SAY_SYLVANA_WALL_02);
                    CastTimer = 1000;
                    HoldTimer = 30000;
                    SetEscortPaused(true);
                    if (Creature* wallTarget = me->SummonCreature(NPC_ICE_WALL,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ(),me->GetOrientation(),TEMPSUMMON_MANUAL_DESPAWN,720000))
                    ...
                    break;
                case 9:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_ESCAPE_01 : SAY_SYLVANA_ESCAPE_01);
                    break;
                case 11:
                    ...
                    break;
                case 12:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_WALL_03 : SAY_SYLVANA_WALL_03);
                    CastTimer = 1000;
                    HoldTimer = 30000;
                    SetEscortPaused(true);
                    if (Creature* wallTarget = me->SummonCreature(NPC_ICE_WALL,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ(),me->GetOrientation(),TEMPSUMMON_MANUAL_DESPAWN,720000))
                    ...
                    break;
                case 13:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_ESCAPE_02 : SAY_SYLVANA_ESCAPE_02);
                    break;
                case 15:
                    ...
                    break;
                case 16:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_WALL_04 : SAY_SYLVANA_WALL_04);
                    CastTimer = 1000;
                    HoldTimer = 30000;
                    SetEscortPaused(true);
                    if (Creature* wallTarget = me->SummonCreature(NPC_ICE_WALL,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ(),me->GetOrientation(),TEMPSUMMON_MANUAL_DESPAWN,720000))
                    ...
                    break;
                case 19:
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_TRAP : SAY_SYLVANA_TRAP);
                    break;
                case 20:
                    if (Creature* wallTarget = m_pInstance->instance->GetCreature(m_uipWallTargetGUID))
                    {
                    ...
                    SetEscortPaused(true);
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, me->GetEntry() == NPC_JAINA_OUTRO ? EMOTE_STATE_READY2HL : EMOTE_STATE_READY1H);
                    break;
                default:
                    break;
            }
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if(me->IsWithinDistInMap(who, 50.0f) && m_pInstance->GetData(DATA_FROSWORN_EVENT) == DONE && m_pInstance->GetData(DATA_PHASE) == 3)
            {
                player = (Player*)who;
                Event = true;
        ...
        void Intro()
        {
            switch(Step)
            {
                case 0:
                    me->RemoveUnitMovementFlag(MOVEMENTFLAG_WALKING);
                    m_lichkingGUID = m_pInstance->GetData64(DATA_LICHKING);
                    lichking = m_pInstance->instance->GetCreature(m_lichkingGUID);
                ...
                case 1:
                    ...
                    if(lichking)
                    {
                        lichking->AI()->Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_LICH_KING_AGGRO_A : SAY_LICH_KING_AGGRO_H);
                ...
                case 7:
                    ...
                    Talk(me->GetEntry() == NPC_JAINA_OUTRO ? SAY_JAINA_AGGRO : SAY_SYLVANA_AGGRO);
                    JumpNextStep(3000);
                    break;
                case 8:
        ...
        void Outro()
        {
            switch(Step)
            {
                case 10:
                    ...
                    break;
                case 11:
                    if (Creature* captain = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_CAPTAINS)))
                        captain->AI()->Talk(SAY_HORDE_FIRE);

                    if(GameObject* cave = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_CAVE)))
                    {
                        Creature* pCaveTarget = me->SummonCreature(NPC_ICE_WALL,cave->GetPositionX(),me->GetPositionY(),me->GetPositionZ(),me->GetOrientation(),TEMPSUMMON_MANUAL_DESPAWN,720000);
                ...
                case 13:
                    ...
                    if (Creature* captain = m_pInstance->instance->GetCreature(m_pInstance->GetData64(DATA_CAPTAINS)))
                    {
                        if (captain->GetEntry() == NPC_BARTLETT)
                        {
                            captain->AI()->Talk(SAY_JAINA_FINAL_1);
                            captain->SummonGameObject(GO_STAIRS_SKYBREAKER, 5223.15f, 1639.64f, 786.649f, 5.5224f, 0, 0, 0.371286f, -0.928518f, 1*DAY);
                            captain->SummonGameObject(GO_STAIRS_SKYBREAKER, 5209.64f, 1625.31f, 799.147f, 5.5224f, 0, 0, 0.371286f, -0.928518f, 1*DAY);
                            captain->SummonGameObject(GO_PORTAL, 5200.53f, 1601.46f, 808.389f, 1.22126f, 0, 0, 0.573384f, 0.819287f, 1*DAY);
                            captain->SummonGameObject(m_pInstance->instance->IsHeroic() ? GO_CAPTAIN_CHEST_4 : GO_CAPTAIN_CHEST_2, 5187.27f, 1611.43f, 808.42f, 0.0965611f, 0, 0, 0.0482618f, 0.998835f, 1*DAY);
                        }
                        else
                        {
                            captain->AI()->Talk(SAY_SYLVANA_FINAL_1);
                            captain->SummonGameObject(GO_STAIRS_ORGRIM_HAMMER, 5245.00f, 1625.00f, 784.00f, 5.8709f, 0, 0, 0.204685f, -0.978828f, 1*DAY);
                            captain->SummonGameObject(GO_STAIRS_ORGRIM_HAMMER, 5236.89f, 1606.70f, 796.60f, 5.8709f, 0, 0, 0.192211f, -0.981354f, 1*DAY);
                            captain->SummonGameObject(GO_STAIRS_ORGRIM_HAMMER, 5228.78f, 1588.40f, 809.20f, 5.8709f, 0, 0, 0.204685f, -0.978828f, 1*DAY);
                            captain->SummonGameObject(GO_PORTAL, 5229.2f, 1566.9f, 819.405f, 4.70851f, 0, 0, 0.708479f, -0.705732f, 1*DAY);
                            captain->SummonGameObject(m_pInstance->instance->IsHeroic() ? GO_CAPTAIN_CHEST_3 : GO_CAPTAIN_CHEST_1, 5219.48f, 1572.98f, 818.975f, 0.0245357f, 0, 0, 0.0122675f, 0.999925f, 1*DAY);
                        }
                    }
                    ...
                    break;
                case 14:
                    Talk(m_pInstance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE ? SAY_JAINA_FINAL_2 : SAY_SYLVANA_FINAL_2);
                    JumpNextStep(20000);
                    break;
                case 15:
        ...
        void UpdateEscortAI(const uint32 diff) override
        {
            ...
            if(m_pInstance->GetData(DATA_LICHKING_EVENT) == SPECIAL && m_pInstance->GetData(DATA_PHASE) != 6) // End Cinematic
            {
            ...
            if (WallCast == true && HoldTimer < 10000 && m_pInstance->GetData(DATA_SUMMONS) == 0)
            {
                ...
                if(GameObject* gate = m_pInstance->instance->GetGameObject(m_uiIceWallGUID))
                    gate->SetGoState(GO_STATE_ACTIVE);
                ++Count;
                switch(Count)
                {
                    case 2:
                        if(GameObject* gate = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_2)))
                        {
                            gate->SetGoState(GO_STATE_READY);
                            if(lichking && lichking->IsAlive())
                                lichking->AI()->Talk(SAY_LICH_KING_WALL_02);
                            m_uiIceWallGUID = gate->GetGUID();
                        }
                        break;
                    case 3:
                        if(GameObject* gate = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_3)))
                        {
                            gate->SetGoState(GO_STATE_READY);
                            if(lichking && lichking->IsAlive())
                                lichking->AI()->Talk(SAY_LICH_KING_WALL_03);
                            m_uiIceWallGUID = gate->GetGUID();
                        }
                        break;
                    case 4:
                        if(GameObject* gate = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_4)))
                        {
                            gate->SetGoState(GO_STATE_READY);
                            if(lichking && lichking->IsAlive())
                                lichking->AI()->Talk(SAY_LICH_KING_WALL_04);
                            m_uiIceWallGUID = gate->GetGUID();
                        }
                        break;
                    case 5:
...
class at_hor_impenetrable_door : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* /*at*/) override
        {
            ...
            if (_instance->GetData(DATA_MARWYN_EVENT) == DONE)
                return true;
