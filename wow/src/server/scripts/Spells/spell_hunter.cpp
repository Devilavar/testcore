...
class spell_hun_masters_call : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_masters_call_SpellScript : public SpellScript
        {
                ...
                if (Unit* target = GetHitUnit())
                    if (Player* caster = GetCaster()->ToPlayer())
                        if (Pet* pet = caster->GetPet())
                        {
                            ...
                            target->RemoveMovementImpairingAuras(); // remove the imparing auras from the target first
                            pet->CastSpell(target, GetEffectValue(), castMask);
                            pet->CastSpell(target, GetSpellInfo()->Effects[EFFECT_0].CalcValue(), castMask); // apply 4s root and snare immunity to pet's target
                            pet->RemoveMovementImpairingAuras(); // remove already applied root and snare from pet
                        }
            }
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_hun_masters_call_SpellScript::CheckPet);
