        ...
        class spell_gen_ribbon_pole_dancer_check_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (Aura* aur = target->GetAura(SPELL_RIBBON_DANCE))
                {
                    ...
                    //if (aur->GetMaxDuration() == 3600000) && target->HasAura(SPELL_HAS_FULL_MIDSUMMER_SET)) - this part doesnt work, need to fix it
...
class spell_gen_enraged_regen_nostack : public SpellScriptLoader
{
    public:
        spell_gen_enraged_regen_nostack() : SpellScriptLoader("spell_gen_enraged_regen_nostack") { }

        class spell_gen_enraged_regen_nostack_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_enraged_regen_nostack_SpellScript);

            SpellCastResult CheckCast()
            {
                if (GetCaster()->HasAura(55694))
                    return SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
                return SPELL_CAST_OK;
            }
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_gen_enraged_regen_nostack_SpellScript::CheckCast);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_gen_enraged_regen_nostack_SpellScript();
        }
};
...
enum GenericScavenge {
    SPELL_SCAVENGE      = 44422,
    GO_FJORD_HAWK_EGG   = 186946,
};

class spell_gen_scavenge : public SpellScriptLoader
{
    public:
        spell_gen_scavenge() : SpellScriptLoader("spell_gen_scavenge") { }

        class spell_gen_scavenge_SpellScript : public SpellScript 
        {
            PrepareSpellScript(spell_gen_scavenge_SpellScript);

            GameObject* hawkEgg;

            bool Validate(SpellInfo const* /*spell*/) override
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_SCAVENGE))
                    return false;
                return true;
            }

            SpellCastResult CheckFjordHawkEgg() 
            {
                Unit* caster = GetCaster();
                hawkEgg = caster->FindNearestGameObject(GO_FJORD_HAWK_EGG, 2.0f);
                if (hawkEgg)
                    return SPELL_CAST_OK;
            ...
            void RemoveFjordHawkEgg() 
            {
                if (hawkEgg)
                    hawkEgg->RemoveFromWorld();
            }

            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_gen_scavenge_SpellScript::CheckFjordHawkEgg);
                AfterHit += SpellHitFn(spell_gen_scavenge_SpellScript::RemoveFjordHawkEgg);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_gen_scavenge_SpellScript();
        }
};
...
void AddSC_generic_spell_scripts()
{
    ...
    new spell_gen_enraged_regen_nostack();
