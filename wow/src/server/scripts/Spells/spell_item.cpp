...
class spell_flag_of_ownership : public SpellScriptLoader
{
    public:
        spell_flag_of_ownership() : SpellScriptLoader("spell_flag_of_ownership") { }

        class spell_flag_of_ownership_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_flag_of_ownership_SpellScript);

            SpellCastResult CheckCorpse()
            {
                if (Unit* unitTarget = GetCaster()->ToPlayer()->GetSelectedUnit())
                {
                    if (!unitTarget->IsFriendlyTo(GetCaster()) && unitTarget->isDead() && unitTarget->GetTypeId() == TYPEID_PLAYER)
                            return SPELL_CAST_OK;
                }
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                GetCaster()->CastSpell(GetCaster()->ToPlayer()->GetSelectedUnit(), 52605, true);
            }

            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_flag_of_ownership_SpellScript::CheckCorpse);
                OnEffectHit += SpellEffectFn(spell_flag_of_ownership_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_flag_of_ownership_SpellScript();
        }
};
