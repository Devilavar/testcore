...
class spell_pal_ardent_defender : public SpellScriptLoader
{
    public:
        ...
        class spell_pal_ardent_defender_AuraScript : public AuraScript
        {
            ...
            bool Load() override
            {
                healPct = (float)GetSpellInfo()->Effects[EFFECT_1].CalcValue() * 0.74f;
            ...
            void Absorb(AuraEffect* aurEff, DamageInfo & dmgInfo, uint32 & absorbAmount)
            {
                ...
                if (remainingHealth <= 0 && !victim->ToPlayer()->HasSpellCooldown(PAL_SPELL_ARDENT_DEFENDER_HEAL))
                {
                    ...
                    uint32 defenseSkillValue = std::min(victim->GetDefenseSkillValue(), (uint32)540);
                    // Max heal when defense skill denies critical hits from raid bosses
                    uint32 reqDefForMaxHeal  = victim->getLevel() * 5;
                    float pctFromDefense = (defenseSkillValue <= reqDefForMaxHeal) ? 0.0f : float(defenseSkillValue) / float(reqDefForMaxHeal);
            ...
            void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (AuraEffect const* aurEff = target->GetAuraEffectOfRankedSpell(SPELL_PALADIN_SANCTIFIED_WRATH_TALENT_R1, EFFECT_2))
                {
					...
					if (target->HasAura(SPELL_PALADIN_FORBEARANCE))
						return;
                    else target->CastCustomSpell(target, SPELL_PALADIN_SANCTIFIED_WRATH, &basepoints, &basepoints, NULL, true, NULL, aurEff);
                }
            }
