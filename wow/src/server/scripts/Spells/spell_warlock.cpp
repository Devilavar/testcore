        ...
        class spell_warl_everlasting_affliction_SpellScript : public SpellScript
        {
            ...
            void HandleScriptEffect(SpellEffIndex /*effIndex*/)
            {
                if (Unit* unitTarget = GetHitUnit())
                    ...
                    {
                        float mod = GetCaster()->HasAura(70947) ? GetCaster()->GetFloatValue(UNIT_MOD_CAST_SPEED) : 1.0f; // Glyph of Quick Decay-- allows haste rolling for curruption
                        aur->GetBase()->SetMaxDuration(18000.0f * mod); // 18s duration by default
                        aur->GetBase()->RefreshDuration();
                        aur->CalculatePeriodic(GetCaster()); // must be called after RefreshDuration()
                    }
            }
