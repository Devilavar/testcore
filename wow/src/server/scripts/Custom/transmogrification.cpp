...
public:
    ...
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "Item Transmogrification", GOSSIP_SENDER_MAIN, TRANSMOG_ACTION_SHOW_TRANSMOGRIFIER);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "Weapon Enchant Transmogrification", GOSSIP_SENDER_MAIN, TRANSMOG_ACTION_SHOW_ENCHANTS);
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "Remove All Transmogrifications", GOSSIP_SENDER_MAIN, TRANSMOG_ACTION_SELECT_REMOVE_EVERYTHING, "Are you sure you wish to remove all transmogrifications?", 0, false);        player->SEND_GOSSIP_MENU(1, creature->GetGUID());
        return true;
    }
        ...
        switch (sender)
        {
            ...
            default: // Transmogrify
            {
                ...
                if (Item* oldItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, sender / 100))
                {
                    ...
                    session->SendNotification(LANG_ERR_NO_EQUIP);
