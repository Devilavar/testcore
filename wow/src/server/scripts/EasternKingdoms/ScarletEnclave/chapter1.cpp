...
public:
    ...
    struct npc_eye_of_acherusAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            me->GetMotionMaster()->MoveCharge(1711.0f, -5820.0f, 147.0f, me->GetSpeed(MOVE_FLIGHT)); //position center
        ...
        {
            if (Unit* controller = me->GetOwner())
               controller->RemoveAurasDueToSpell(SPELL_EYE_CONTROL);
        }
    };
};
...
class spell_recall_eye_of_acherus : public SpellScriptLoader
{
    public:
        spell_recall_eye_of_acherus() : SpellScriptLoader("spell_recall_eye_of_acherus") { }

        class spell_recall_eye_of_acherus_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_recall_eye_of_acherus_SpellScript);

            void HandleDummy()
            {
                if (GetCaster()->GetTypeId() == TYPEID_UNIT && GetCaster()->GetEntry() == 28511)
                {
                    if (GetCaster()->GetOwner())
                        GetCaster()->GetOwner()->RemoveAurasDueToSpell(SPELL_EYE_CONTROL);
            ...
            {
                OnCast += SpellCastFn(spell_recall_eye_of_acherus_SpellScript::HandleDummy);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_recall_eye_of_acherus_SpellScript();
        }
};
...
void AddSC_the_scarlet_enclave_c1()
{
    ...
    new spell_recall_eye_of_acherus();
}
