...
void OPvPCapturePointNA::SpawnNPCsForTeam(uint32 team)
{
    ...
    {
        DelCreature(i); // precaution
        if (!AddCreature(i, creatures[i].entry, creatures[i].map, creatures[i].x, creatures[i].y, creatures[i].z, creatures[i].o, OutdoorPvP::GetTeamIdByTeam(team), 1000000))
            TC_LOG_ERROR("misc", "Halaa: Failed to spawn creature: %u", creatures[i].entry);
    }
}
...
m_HalaaState(HALAA_N), m_GuardCheckTimer(NA_GUARD_CHECK_TIME)
{
