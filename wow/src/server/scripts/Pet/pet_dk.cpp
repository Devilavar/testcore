...
class npc_pet_dk_ebon_gargoyle : public CreatureScript
{
    public:
        ...
        struct npc_pet_dk_ebon_gargoyleAI : CasterAI
        {
            ...
            void Reset()
            {
                if (Unit* target = ObjectAccessor::GetUnit(*me, me->GetOwner()->GetTarget()))
                    AttackStart(target);
                else
                {
                    ...
                    {
                        if ((*iter)->HasAura(SPELL_DK_SUMMON_GARGOYLE_1, me->GetOwnerGUID()))
                        {
                            AttackStart(*iter);
                            break;
                        }
            ...
            void SpellHit(Unit* source, SpellInfo const* spell) override
            {
                ...
                _despawnTimer = 6 * IN_MILLISECONDS;
            }
