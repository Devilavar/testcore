...
public:
    ...
    ChatCommand* GetCommands() const override
    {
        static ChatCommand listCommandTable[] =
        {
            ...
            { "combat",   rbac::RBAC_PERM_COMMAND_LIST_COMBAT,  true,  &HandleListCombatCommand,   "", NULL },
    ...
    static bool HandleListCombatCommand(ChatHandler* handler, char const* args)
    {
        ...
        // blue
        handler->PSendSysMessage("|cff0019ff%u | %u | %u", uint32(target->getAttackers().size()), target->GetCombatTimer(), target->getHostileRefManager().isEmpty() ? 1 : 0);

        std::list<HostileReference*> const& threatList = target->getThreatManager().getThreatList();
        for (std::list<HostileReference*>::const_iterator itr = threatList.begin(); itr != threatList.end(); ++itr)
            if (Unit* refTarget = (*itr)->getTarget()) // purple
                handler->PSendSysMessage("|cffaa00ff%u | %u | %u", refTarget->GetTypeId() == TYPEID_UNIT ? refTarget->GetEntry() : 0, refTarget->GetGUIDLow(), (refTarget->IsGuardian() || refTarget->IsPet()) ? 1 : 0);
        ...
        while (ref)
        {
            if (Unit* unit = ref->GetSource()->GetOwner())
            {
                ++count;
                // green
                handler->PSendSysMessage("|cff317f05%u | %u | %u | %u | %f", count, unit->GetTypeId() == TYPEID_UNIT ? unit->GetEntry() : 0, unit->GetGUIDLow(), (unit->IsGuardian() || unit->IsPet()) ? 1 : 0, ref->getThreat());

                if (unit->IsGuardian() || unit->IsPet())
                    handler->PSendSysMessage("|cff317f05----%f | %u | %u | %i", unit->GetDistance(target),  unit->GetOwner()->GetGUIDLow(), unit->isDead() ? 1 : 0, unit->GetOwner()->isDead() ? 1 : 0, unit->GetCharmInfo() ? unit->GetCharmInfo()->GetCommandState() : -1);
            }
            ref = ref->next();
        }
        ...
        for (Unit::AttackerSet::const_iterator itr = target->getAttackers().begin(); itr != target->getAttackers().end(); ++itr)
            if (*itr) // red
                handler->PSendSysMessage("|cffbc4242%u | %u | %u", (*itr)->GetTypeId() == TYPEID_UNIT ? (*itr)->GetEntry() : 0, (*itr)->GetGUIDLow(), ((*itr)->IsGuardian() || (*itr)->IsPet()) ? 1 : 0);
        return true;
    }
