...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand reloadCommandTable[] =
        {
            ...
            { "creature_template_outfits",     rbac::RBAC_PERM_COMMAND_RELOAD_CREATURE_OUTFITS,                 true,  &HandleReloadCreatureTemplateOutfitsCommand,    "", NULL },
            ...
            { "itemextendedcost",              rbac::RBAC_PERM_COMMAND_RELOAD_ITEMEXTENDEDCOST,                 true,  &HandleReloadItemExtendedCost,                  "", NULL },
    ...
    static bool HandleReloadItemExtendedCost(ChatHandler* handler, const char* /*args*/)
    {
        TC_LOG_INFO("misc", "Re-Loading `itemextendedcost_dbc` Table!");
        sItemExtendedCostStore.Clear();
        uint32 availableDbcLocales = 0xFFFFFE00;
        std::string dbcPath = sWorld->GetDataPath()+"dbc/";
        std::string const& filename = "ItemExtendedCost.dbc";

        std::string dbcFilename = dbcPath + filename;
        SqlDbc* sql = new SqlDbc(&filename, &CustomItemExtendedCostfmt, &CustomItemExtendedCostIndex, sItemExtendedCostStore.GetFormat());

        if (sItemExtendedCostStore.Load(dbcFilename.c_str(), sql))
        {
            for (uint8 i = 0; i < TOTAL_LOCALES; ++i)
            {
                ...
                if (!sItemExtendedCostStore.LoadStringsFrom(localizedName.c_str()))
        ...
        else
        {
            ...
            if (FILE* f = fopen(dbcFilename.c_str(), "rb"))
            {
                ...
                stream << dbcFilename << " exists, and has " << sItemExtendedCostStore.GetFieldCount() << " field(s) (expected " << strlen(sItemExtendedCostStore.GetFormat()) << "). Extracted file might be from wrong client version or a database-update has been forgotten.";
                std::string buf = stream.str();
                fclose(f);
                handler->SendGlobalGMSysMessage(buf.c_str());
                handler->SetSentErrorMessage(true);
                delete sql;
                return false;
            }
            ...
            {
                handler->SendGlobalGMSysMessage("Error: itemextendedcost_dbc doesn't exist");
                handler->SetSentErrorMessage(true);
                delete sql;
                return false;
            }
        ...
        handler->SendGlobalGMSysMessage("DB table `itemextendedcost_dbc` reloaded.");
        return true;
    }
