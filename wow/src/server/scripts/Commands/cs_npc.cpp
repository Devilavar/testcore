...
public:
    ...
    static bool HandleNpcAddVendorItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!sObjectMgr->IsVendorItemValid(vendor_entry, itemId, maxcount, incrtime, extendedcost, sort, handler->GetSession()->GetPlayer()))
        {
            handler->SetSentErrorMessage(true);
            return false;
        }

        sObjectMgr->AddVendorItem(vendor_entry, itemId, maxcount, incrtime, extendedcost, sort);
    ...
    static bool HandleNpcDeleteVendorItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!sObjectMgr->RemoveVendorItem(vendor->GetEntry(), itemId, sort))
        {
