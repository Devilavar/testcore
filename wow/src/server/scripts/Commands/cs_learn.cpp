...
public:
    ...
    static bool HandleLearnCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->extractOptFirstArg((char*)args, &spellStr, &nameStr); 
        ...
        uint32 spell = handler->extractSpellIdFromLink(spellStr);
        ...
        if(nameStr)
        {
            ...
            if(!targetPlayer) //offline
            {
                ...
                QueryResult result1 = CharacterDatabase.PQuery("SELECT `spell` FROM `character_spell` WHERE `guid` = %u AND `spell` = %u", guid, spell);

                if(result1)
                {
                    handler->PSendSysMessage(LANG_TARGET_KNOWN_SPELL, nameStr);
                    handler->SetSentErrorMessage(true);

                    return false;
                }

                CharacterDatabase.PExecute("INSERT INTO `character_spell` (`guid`, `spell`) VALUES (%u, %u);", guid, spell);
