...
public:
    ...
    {
        static ChatCommand graveyardCommandTable[] =
        {
            { "add",            rbac::RBAC_PERM_COMMAND_GRAVEYARD_ADD,  false,  &HandleGraveyardAddCommand,         "", NULL },
        ...
        static ChatCommand commandTable[] =
        {
            ...
            { "graveyard",        rbac::RBAC_PERM_COMMAND_GRAVEYARD,        false, NULL,                           "", graveyardCommandTable },
    ...
    static bool HandleGraveyardAddCommand(ChatHandler* handler, char const* args)
    {
        uint16 teamval = 0;
        std::string argstr = (char*)args;
        ...
        if (!creature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), map, chr->GetPhaseMaskForSpawn(), 6491, x, y, z, o))
        {
            delete creature;
            return false;
        }
        ...
        uint32 zoneId       = sMapMgr->GetZoneId(map->GetId(), x, y, z);

        PreparedStatement* stmt = WorldDatabase.GetPreparedStatement(WORLD_INS_GRAVEYARD);

        stmt->setInt32(0, map->GetId());
        stmt->setFloat(1, float(x));
        stmt->setFloat(2, float(y));
        stmt->setFloat(3, float(z));

        WorldDatabase.Execute(stmt);

        QueryResult result = WorldDatabase.PQuery("SELECT max(ID) FROM `worldsafelocs_dbc`;");

        Field* field = result->Fetch();

        int32 ID = field[0].GetInt32();

        PreparedStatement* stmt2 = WorldDatabase.GetPreparedStatement(WORLD_INS_GRAVEYARD_ZONE);

        stmt2->setInt32(0, ID + 1);
        stmt2->setUInt32(1, zoneId);
        stmt2->setUInt16(2, teamval);

        WorldDatabase.Execute(stmt2);
        //sObjectMgr->LoadGraveyardZones();
        handler->PSendSysMessage("New graveyard added; server must restart for it to take effect!");
        return true;
    }
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (playerTarget->getLevel() < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
            ...
            for (ObjectMgr::QuestMap::const_iterator iter = qTemplates.begin(); iter != qTemplates.end(); ++iter)
            {
                ...
                if (qInfo->GetMinLevel() <= playerTarget->getLevel())
                    continue;

                for (uint32 i = 0; i < QUEST_REWARDS_COUNT; ++i)
                    if (qInfo->RewardItemId[i] == itemId)
                    {
                        meetsreq = false;
                        break;
                    }
                ...
                for (uint32 i = 0; i < QUEST_REWARD_CHOICES_COUNT; ++i)
                    if (qInfo->RewardChoiceItemId[i] == itemId)
                    {
                        meetsreq = false;
                        break;
                    }
            ...
            if (!meetsreq)
            {
                handler->PSendSysMessage("Player does not meet the requirements to obtain this item.");
                handler->SetSentErrorMessage(true);
                return false;
            }
        }
        ...
        if (player == playerTarget && item)
            item->SetBinding(true); // make the item bind to them
    ...
    static bool HandleSetSkillCommand(ChatHandler* handler, char const* args)
    {
       if(!args)
         ...
         char* levelStr;
         char* skillStr;
         handler->extractOptFirstArg((char*)args, &skillStr, &levelStr); 

         char const* maxPureSkill = strtok(NULL, " ");
         char const* nameStr = strtok(NULL, " ");
         ...
         if (skill <= 0)
         {
          ...
          if(maxPureSkill && isNumeric(maxPureSkill))
          {
              maxLevel = atoi(maxPureSkill);
          }
          else maxLevel = 450;
          ...
          if(nameStr != NULL)
          {
              targetPlayer = sObjectAccessor->FindPlayerByName(nameStr);
              ...
              if(!targetPlayer) //offline
              {
                 ...
                 if(!results || results->GetRowCount() == 0)
                  ...
                  CharacterDatabase.PExecute("REPLACE INTO `character_skills` (`guid`, `skill`, `value`, `max`) VALUES (%u, %u, %u, %u);", guid, skill, level, maxLevel);
          ...
          std::string tNameLink = handler->GetNameLink(targetPlayer);

          if (!targetPlayer->GetSkillValue(skill))
          {
          ...
          targetPlayer->SetSkill(skill, targetPlayer->GetSkillStep(skill), level, level);
          handler->PSendSysMessage(LANG_SET_SKILL, skill, skillLine->name[handler->GetSessionDbcLocale()], tNameLink.c_str(), level, level);
    ...
    */
    ...
    static bool HandleDamageCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!spellStr)
        {
            ...
            handler->GetSession()->GetPlayer()->CalcAbsorbResist(target, schoolmask, SPELL_DIRECT_DAMAGE, damage, &absorb, &resist, NULL, -1);
