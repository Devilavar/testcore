...
enum creatureData
{
    NELSON_THE_NICE = 14536,
    SOLENOR_THE_SLAYER = 14530,

    CREEPING_DOOMS = 14761,
    SPELL_DREADFUL_FRIGHT = 23275,
};

enum events
{
    EVENT_SUMMON_CREEPING_DOOMS = 1,
    EVENT_DREADFUL_FRIGHT,
};

class npc_nelson_the_nice : public CreatureScript
{
public:
    npc_nelson_the_nice() : CreatureScript("npc_nelson_the_nice") { }

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_nelson_the_niceAI(creature);
    }

    struct npc_nelson_the_niceAI : public ScriptedAI
    {
        npc_nelson_the_niceAI(Creature* creature) : ScriptedAI(creature) { }

        void Reset() override
        {
            _nelsonguid = 0;
            if (me->GetEntry() == SOLENOR_THE_SLAYER)
            {
                events.Reset();
                events.ScheduleEvent(EVENT_SUMMON_CREEPING_DOOMS, urand(5000, 7000));
                events.ScheduleEvent(EVENT_DREADFUL_FRIGHT, urand(15000, 20000));
            }
        }
        ...
        void EnterCombat(Unit* who) override
        {
            if (me->GetEntry() == NELSON_THE_NICE)
            {
                if (TempSummon* solenor = me->SummonCreature(SOLENOR_THE_SLAYER, me->GetPosition(), TEMPSUMMON_MANUAL_DESPAWN))
                {
                    solenor->AI()->AttackStart(who);
                    solenor->AI()->SetGUID(me->GetGUID());
        ...
        void EnterEvadeMode() override
        {
            if (me->GetEntry() == SOLENOR_THE_SLAYER)
            {
                if (Unit* nelson = ObjectAccessor::GetUnit(*me, _nelsonguid))
                {
                    nelson->ToCreature()->Respawn();
                    float x, y, z;
                    nelson->ToCreature()->GetRespawnPosition(x, y, z);
                    nelson->GetMotionMaster()->MovePoint(0, x, y, z);
                    me->DespawnOrUnsummon();
                }
            }
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SUMMON_CREEPING_DOOMS:
                    {
                        float x, y, z;
                        me->GetNearPoint(me, x, y, z, me->GetObjectSize(), 2.0f, me->GetOrientation());
                        if (TempSummon* creepingdoom = me->SummonCreature(CREEPING_DOOMS, x, y, z, me->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 1000))
                        {
                            creepingdoom->setFaction(me->getFaction());
                            creepingdoom->SetReactState(REACT_AGGRESSIVE);
                            creepingdoom->SetSpeed(MOVE_RUN, 0.5f);
                            creepingdoom->SetSpeed(MOVE_WALK, 0.5f);
                            creepingdoom->AI()->AttackStart(me->GetVictim());
                        }
                        events.ScheduleEvent(EVENT_SUMMON_CREEPING_DOOMS, urand(5000, 7000));
                        break;
                    }
                    case EVENT_DREADFUL_FRIGHT:
                        DoCastVictim(SPELL_DREADFUL_FRIGHT);
                        events.ScheduleEvent(EVENT_DREADFUL_FRIGHT, urand(15000, 20000));
                        break;
                }
            }
