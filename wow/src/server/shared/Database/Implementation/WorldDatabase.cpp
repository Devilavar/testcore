...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_INS_NPC_VENDOR, "INSERT INTO npc_vendor (entry, item, maxcount, incrtime, extendedcost, sort) VALUES(?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(WORLD_DEL_NPC_VENDOR, "DELETE FROM npc_vendor WHERE entry = ? AND item = ? AND sort = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_SEL_NPC_VENDOR_REF, "SELECT item, maxcount, incrtime, ExtendedCost, sort FROM npc_vendor WHERE entry = ? ORDER BY slot, sort ASC", CONNECTION_SYNCH);
    ...
    PrepareStatement(WORLD_INS_GRAVEYARD, "INSERT INTO worldsafelocs_dbc (map, x, y, z) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    // 0: uint8
