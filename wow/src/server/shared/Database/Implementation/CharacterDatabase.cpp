...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
                     ...
                     "health, power1, power2, power3, power4, power5, power6, power7, instance_id, speccount, activespec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, deathTime "
    ...
    PrepareStatement(CHAR_REP_ITEM_INSTANCE, "REPLACE INTO item_instance (itemEntry, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, TransmogEntry, TransmogEnchant, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_INSTANCE, "UPDATE item_instance SET itemEntry = ?, owner_guid = ?, creatorGuid = ?, giftCreatorGuid = ?, count = ?, duration = ?, charges = ?, flags = ?, enchantments = ?, randomPropertyId = ?, durability = ?, playedTime = ?, text = ?, TransmogEntry = ?, TransmogEnchant = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_ARENA_TEAM_RANK_I, "UPDATE arena_team SET rank = rank + 1 WHERE type = ? AND rating > ? AND rank >= ? AND arenaTeamId <> ? AND rank <> 0", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ARENA_TEAM_RANK_D, "UPDATE arena_team SET rank = rank - 1 WHERE type = ? AND rating <= ? AND rank <= ? AND arenaTeamId <> ? AND rank <> 0", CONNECTION_ASYNC);
    ...
    // Shadez Armory
    PrepareStatement(CHAR_DEL_ARMORY_CHAR_STATS, "DELETE FROM armory_character_stats WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER, "UPDATE characters SET name=?,race=?,class=?,gender=?,level=?,xp=?,money=?,playerBytes=?,playerBytes2=?,playerFlags=?,"
                     ...
                     "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,online=?,deathTime=? WHERE guid=?", CONNECTION_ASYNC);
