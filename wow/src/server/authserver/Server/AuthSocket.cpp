...
AuthSocket::AuthSocket(RealmSocket& socket) :
    ...
    _expversion(0), _accountSecurityLevel(SEC_PLAYER), _OnReadRequests(0),
    _LogonChallengeRequests(0), _LogonProofRequests(0), _ReconnectChallengeRequests(0),
     _ReconnectProofRequests(0), _RealmListRequests(0)
{
...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        {
            ++_OnReadRequests;
            if (_OnReadRequests > 200)
            {
                TC_LOG_ERROR("server.authserver", "AuthSocket::OnRead: User: %s [IP: %s] exceeded maximum requests, possible ongoing DoS attempt", _login.c_str(), socket().getRemoteAddress().c_str());
                socket().shutdown();
                return;
            }
        }
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    ++_LogonChallengeRequests;
    if (_LogonChallengeRequests > 200)
    {
        TC_LOG_ERROR("server.authserver", "AuthSocket::_HandleLogonChallenge: User: %s [IP: %s] exceeded maximum requests, possible ongoing DoS attempt", _login.c_str(), socket().getRemoteAddress().c_str());
        socket().shutdown();
        return false;
    }
    ...
    _build = ch->build;
    _expversion = uint8(GetExpansionFlag(_build));
        ...
        if (res2)
        {
            ...
            if (!locked)
            {
                ...
                else
                {
                    ...
                    _accountSecurityLevel = fields[5].GetUInt8();
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    ++_LogonProofRequests;
    if (_LogonProofRequests > 200)
    {
        TC_LOG_ERROR("server.authserver", "AuthSocket::_HandleLogonProof: User: %s [IP: %s] exceeded maximum requests, possible ongoing DoS attempt", _login.c_str(), socket().getRemoteAddress().c_str());
        socket().shutdown();
        return false;
    }
    ...
    if (!memcmp(M.AsByteArray().get(), lp.M1, 20))
    {
        ...
        if (_expversion > VANILLA_EXP_FLAG)
        {
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    else
                    {
                        ...
                        uint32 acc_id = (*loginfail)[0].GetUInt32();
                        stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_FAILED_LOGINS);
                        stmt->setUInt32(0, acc_id);
                        LoginDatabase.Execute(stmt);
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    ++_ReconnectChallengeRequests;
    if (_ReconnectChallengeRequests > 200)
    {
        TC_LOG_ERROR("server.authserver", "AuthSocket::_HandleReconnectChallenge: User: %s [IP: %s] exceeded maximum requests, possible ongoing DoS attempt", _login.c_str(), socket().getRemoteAddress().c_str());
        socket().shutdown();
        return false;
    }
    ...
    _accountSecurityLevel = fields[2].GetUInt8();
...
bool AuthSocket::_HandleReconnectProof()
{
    ...
    ++_ReconnectProofRequests;
    if (_ReconnectProofRequests > 200)
    {
        TC_LOG_ERROR("server.authserver", "AuthSocket::_HandleReconnectProof: User: %s [IP: %s] exceeded maximum requests, possible ongoing DoS attempt", _login.c_str(), socket().getRemoteAddress().c_str());
        socket().shutdown();
        return false;
    }
...
bool AuthSocket::_HandleRealmList()
{
    ...
    ++_RealmListRequests;
    if (_RealmListRequests > 200)
    {
        TC_LOG_ERROR("server.authserver", "AuthSocket::_HandleRealmList: User: %s [IP: %s] exceeded maximum requests, possible ongoing DoS attempt", _login.c_str(), socket().getRemoteAddress().c_str());
        socket().shutdown();
        return false;
    }
    ...
    bool ShowOtherBuilds = sConfigMgr->GetBoolDefault("RealmList.ShowOtherBuilds", true);

    size_t RealmListSize = 0;
    for (RealmList::RealmMap::const_iterator i = sRealmList->begin(); i != sRealmList->end(); ++i)
    {
        const Realm &realm = i->second;
        // don't work with realms which are not compatible with the client
        bool okBuild = CanConnectTo(_build, i->second.gamebuild, _expversion);

        if (!okBuild && !ShowOtherBuilds)
            continue;

        // No SQL injection. id of realm is controlled by the database.
        uint32 flag = realm.flag;
        RealmBuildInfo const* buildInfo = GetBuildInfo(i->second.gamebuild);
        if (!okBuild)
        {
            if (!buildInfo)
                continue;
        ...
        else if (_build != i->second.gamebuild && buildInfo)
        ...
        if (_expversion == VANILLA_EXP_FLAG && flag & REALM_FLAG_SPECIFYBUILD)
        {
