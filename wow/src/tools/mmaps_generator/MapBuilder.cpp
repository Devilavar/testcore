...
namespace MMAP
{
    MapBuilder::MapBuilder(float maxWalkableAngle, bool skipLiquid,
        ...
        bool debugOutput, bool bigBaseUnit, bool overwrite, const char* offMeshFilePath) :
    ...
    void MapBuilder::buildMap(uint32 mapID)
    {
        ...
        if (!tiles->empty())
        {
            ...
            int32 count = 0, lastpct = 0;
            printf("[Map %03i] Processing.... 0%%\n", mapID);
            for (std::set<uint32>::iterator it = tiles->begin(); it != tiles->end(); ++it)
            {
                ...
                int currpct = (int)floorf(100.0f * ((float)count / (float)tiles->size()));
                if (currpct != lastpct)
                    printf("[Map %03i] Processing.... %i%%\n", mapID, currpct);
        ...
        dtNavMesh* navMesh)
    {
        ...
        config.cs = 0.1333333f;
        config.ch = 0.2666666f;

        // All are in UNIT metrics!
        const static int VERTEX_PER_MAP = int(GRID_SIZE/config.cs + 0.5f);
        const static int VERTEX_PER_TILE = m_bigBaseUnit ? 40 : 80; // must divide VERTEX_PER_MAP
        const static int TILES_PER_MAP = VERTEX_PER_MAP/VERTEX_PER_TILE;

        config.walkableSlopeAngle = 50.0f;                         // maximum slope angle that the agent can climb
        config.tileSize = VERTEX_PER_TILE;
        config.walkableRadius = (int)ceilf(0.1f / config.cs);      // minimum radius of an area that the agent can go
        config.borderSize = config.walkableRadius + 3;             // unk
        config.maxEdgeLen = VERTEX_PER_TILE + 1;                   // maximum length of a single edge of a poly, ideally should be less than or equal to tileSize
        config.walkableHeight = (int)ceilf(2.0f / config.ch);      // height of the agent
        config.walkableClimb = (int)floorf(1.8f / config.ch);      // height that the agent can climb
        config.minRegionArea = rcSqr(2);                           // minimum size an ordinary poly can be
        config.mergeRegionArea = rcSqr(1);                         // should be less than minRegionArea
        config.maxSimplificationError = 1.8f;                      // maximum error that an edge of a poly can have before it makes a new poly
        config.detailSampleDist = config.cs * 1;                   // determines the distance between possible polygons
        config.detailSampleMaxError = config.ch * 2;               // maximum error that a sample can have, typically doesn't have any impact on quality of mesh
        ...
        params.walkableClimb = config.ch * config.walkableClimb;      // keep less that walkableHeight (aka agent height)!
